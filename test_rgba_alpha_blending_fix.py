...
def test_rgba_alpha_blending_fix():
    ...
    rgba_red = torch.tensor([[[1.0, 1.0], [1.0, 1.0]],   # R channel (red)
                            [[0.0, 0.0], [0.0, 0.0]],   # G channel
                            [[0.0, 0.0], [0.0, 0.0]],   # B channel  
                            [[0.5, 0.5], [0.5, 0.5]]])  # A channel (50% transparent)
    
    print(f"Input: Semi-transparent red with 50% alpha")
    print(f"RGBA values: R=1.0, G=0.0, B=0.0, A=0.5")
    ...
    if (abs(r_val - expected_r) < tolerance and 
        abs(g_val - expected_g) < tolerance and 
        abs(b_val - expected_b) < tolerance):
        ...
        print(f"Expected: R=1.0, G=0.5, B=0.5")
...
def test_backward_compatibility():
    ...
    try:
        from kornia.color.rgb import rgba_to_rgb, rgb_to_rgba
    except ImportError:
