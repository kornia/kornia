...
Tests actually run under the repo `.venv` (`pixi run test`/`test-quick`/etc. all shell out to `uv run pytest`, which uses `.venv`, currently torch 2.9.1) — the separate `.pixi` conda environment is not what test invocations use and its package versions should not be assumed to match `.venv`.
