...
from kornia.core import Device, Dtype, Size, Tensor, as_tensor, ones, stack, tensor, zeros
...
class AffineGenerator3D(RandomGeneratorBase):
    ...
    def make_samplers(self, device: Device = None, dtype: Dtype = None) -> None:
        ...
        self._translate: Optional[Tensor] = None
        if self.translate is not None:
            self._translate = as_tensor(self.translate, device=device, dtype=dtype)
            _singular_range_check(self._translate, 'translate', bounds=(0, 1), mode='3d')

        # check scale range
        self._scale: Optional[Tensor] = None
        if self.scale is not None:
            _scale = as_tensor(self.scale, device=device, dtype=dtype)
            if _scale.shape == Size([2]):
                self._scale = _scale.unsqueeze(0).repeat(3, 1)
            elif _scale.shape != Size([3, 2]):
    ...
    def forward(self, batch_shape: Size, same_on_batch: bool = False) -> Dict[str, Tensor]:
        ...
        if self._scale is not None:
            ...
            scale = ones(batch_size, device=_device, dtype=_dtype).reshape(batch_size, 1).repeat(1, 3)

        if self._translate is not None:
            max_dx: Tensor = self._translate[0] * width
            max_dy: Tensor = self._translate[1] * height
            max_dz: Tensor = self._translate[2] * depth
            ...
            translations = zeros((batch_size, 3), device=_device, dtype=_dtype)

        # center should be in x,y,z
        center: Tensor = tensor([width, height, depth], device=_device, dtype=_dtype).view(1, 3) / 2.0 - 0.5
        ...
        if self.shears is not None:
            ...
            sxy = sxz = syx = syz = szx = szy = tensor([0] * batch_size, device=_device, dtype=_dtype)

        return dict(
            translations=as_tensor(translations, device=_device, dtype=_dtype),
            center=as_tensor(center, device=_device, dtype=_dtype),
            scale=as_tensor(scale, device=_device, dtype=_dtype),
            angles=as_tensor(angles, device=_device, dtype=_dtype),
            sxy=as_tensor(sxy, device=_device, dtype=_dtype),
            sxz=as_tensor(sxz, device=_device, dtype=_dtype),
            syx=as_tensor(syx, device=_device, dtype=_dtype),
            syz=as_tensor(syz, device=_device, dtype=_dtype),
            szx=as_tensor(szx, device=_device, dtype=_dtype),
            szy=as_tensor(szy, device=_device, dtype=_dtype),
        )
...
def random_affine_generator3d(
    ...
    Args:
        ...
        degrees (Tensor): Ranges of degrees with shape (3, 2) for yaw, pitch and roll.
        translate (Tensor, optional):  maximum absolute fraction with shape (3,) for horizontal, vertical
    ...
    if scale is not None:
        ...
        scale = ones(batch_size, device=device, dtype=dtype).reshape(batch_size, 1).repeat(1, 3)

    if translate is not None:
        if translate.shape != Size([3]):
        ...
        max_dz: Tensor = translate[2] * depth
        # translations should be in x,y,z
        translations = stack(
            [
        ...
        translations = zeros((batch_size, 3), device=device, dtype=dtype)

    # center should be in x,y,z
    center: Tensor = tensor([width, height, depth], device=device, dtype=dtype).view(1, 3) / 2.0 - 0.5
    center = center.expand(batch_size, -1)

    if shears is not None:
        if shears.shape != Size([6, 2]):
        ...
        sxy = sxz = syx = syz = szx = szy = tensor([0] * batch_size, device=device, dtype=dtype)
