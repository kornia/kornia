...
def random_perspective_generator(
    ...
    if not (type(height) == int and height > 0 and type(width) == int and width > 0):
        raise ValueError(f"'height' and 'width' must be integers. Got {height}, {width}.")
...
def random_affine_generator(
    ...
    if scale is not None:
        ...
        if not (len(scale.shape) == 1 and (len(scale) == 2 or len(scale) == 4)):
            raise ValueError(f"`scale` shall have 2 or 4 elements. Got {scale}.")
    ...
    if translate is not None:
        ...
        if not (0.0 <= translate[0] <= 1.0 and 0.0 <= translate[1] <= 1.0 and translate.shape == torch.Size([2])):
            raise ValueError(f"Expect translate contains two elements and ranges are in [0, 1]. Got {translate}.")
...
def random_crop_generator(
    ...
    if not (input_size[0] > 0 and input_size[1] > 0 and (size > 0).all()):
        raise ValueError(f"Got non-positive input size or size. {input_size}, {size}.")
    size = size.floor()
    ...
    if resize_to is None:
        ...
        if not (
            ...
            raise ValueError(f"`resize_to` must be a tuple of 2 positive integers. Got {resize_to}.")
...
def random_crop_size_generator(
    ...
    if not (len(size) == 2 and type(size[0]) == int and size[1] > 0 and type(size[1]) == int and size[1] > 0):
        raise ValueError(f"'height' and 'width' must be integers. Got {size}.")
...
def random_rectangles_params_generator(
    ...
    if not (isinstance(value, (int, float)) and value >= 0 and value <= 1):
        raise ValueError(f"'value' must be a number between 0 - 1. Got {value}.")
...
def center_crop_generator(
    ...
    if not (height >= size[0] and width >= size[1]):
        raise ValueError(f"Crop size must be smaller than input size. Got ({height}, {width}) and {size}.")
...
def random_motion_blur_generator(
    ...
    if isinstance(kernel_size, int):
        ...
        if len(kernel_size) != 2:
            raise ValueError(f"`kernel_size` must be (2,) if it is a tuple. Got {kernel_size}.")
        ksize_factor = (
...
def random_cutmix_generator(
    ...
    if not (num_mix >= 1 and isinstance(num_mix, (int,))):
        raise ValueError(f"`num_mix` must be an integer greater than 1. Got {num_mix}.")
    if not (type(height) == int and height > 0 and type(width) == int and width > 0):
        raise ValueError(f"'height' and 'width' must be integers. Got {height}, {width}.")
