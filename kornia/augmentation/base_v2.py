...
class _BasicAugmentationBase(nn.Module):
    ...
    def __check_batching__(self, input: TensorWithTransformMat) -> None:
...
class AugmentationBase(_BasicAugmentationBase):
    ...
    def __init__(
        ...
        if self.return_transform and hasattr(self, "augmentation_type"):
            raise RuntimeError(
                "Operation has to implement `Invertible` interface in order to return transformation matrix.")
...
class Invertible2D:
    ...
    def compute_transformation(self, input: torch.Tensor, params: Dict[str, torch.Tensor]) -> torch.Tensor:
        if self.augmentation_type == 'intensity':
