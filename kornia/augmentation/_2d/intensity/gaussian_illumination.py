...
class RandomGaussianIllumination(IntensityAugmentationBase2D):
    ...
    Examples:
        ...
        >>> aug(input)
        tensor([[[[0.5000, 0.4791, 0.4623],
                  [0.3575, 0.2725, 0.2042],
                  [0.2448, 0.1091, 0.0000]],
        <BLANKLINE>
                 [[0.5000, 0.4791, 0.4623],
                  [0.3575, 0.2725, 0.2042],
                  [0.2448, 0.1091, 0.0000]],
        <BLANKLINE>
                 [[0.5000, 0.4791, 0.4623],
    ...
    def __init__(
        ...
        sigma: Optional[Union[float, Tuple[float, float]]] = (0.25, 1.0),
        sign: Optional[Union[float, Tuple[float, float]]] = (-1, 1.0),
        p: float = 0.5,
        ...
        if isinstance(sign, (tuple, float)):
            if isinstance(sign, float):
                ...
                sign = (sign[0], sign[0])
            elif len(sign) > 2 or len(center) <= 0:
                raise ValueError(
                    "The length of amount must be greater than 0 \
                        and less than or equal to 2, and it should be a tuple."
                )
            ...
            raise ValueError("amount must be a tuple or a float")
        KORNIA_CHECK(
            all(-1 <= el <= 1 for el in center),
            "center of gaussian value must be between -1 and 1.",
        )
