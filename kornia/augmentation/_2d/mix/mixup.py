...
class RandomMixUpV2(MixAugmentationBaseV2):
    ...
    def apply_transform(
        ...
        # `mixup_lambdas` is generated by MixupGenerator without necessarily matching `input`'s
        # dtype (its own device/dtype tracking is independent of the real runtime input) -- without
        # an explicit dtype= here, `input * (1 - lam)` silently upcasts a float16/bfloat16 `input`
        # to float32 through ordinary type promotion the moment `lam` isn't already float32.
        lam = params["mixup_lambdas"].view(-1, 1, 1, 1).expand_as(input).to(device=input.device, dtype=input.dtype)
