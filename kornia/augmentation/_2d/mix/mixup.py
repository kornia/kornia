    ...
    def apply_non_transform_class(
        ...
        out_labels = torch.stack(
            [
                ...
                input.to(device=input.device, dtype=DType.to_torch(params["dtype"].item())),
                torch.zeros((len(input),), device=input.device, dtype=DType.to_torch(params["dtype"].item())),
            ],
            dim=-1,
        )
        return out_labels
    ...
    def apply_transform_class(
        ...
        out_labels = torch.stack(
            [
                ...
                params["mixup_lambdas"].to(device=input.device, dtype=DType.to_torch(params["dtype"].item())),
            ],
            dim=-1,
        )
        return out_labels
