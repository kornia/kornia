...
class RandomCutMixV2(MixAugmentationBaseV2):
    ...
    Convention:
        ...
        - ``p`` is a batch-wide gate and is applied once: one draw per call selects the whole batch with probability
          ...
          a cut and none is dropped again. With ``same_on_batch=False`` the cut geometries are not required to match;
          ``same_on_batch=True`` shares one geometry across all rows and mixes. A cut can still leave the image
          unchanged through self-pairing or a zero-sized cut.
