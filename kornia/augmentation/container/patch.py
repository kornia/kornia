    ...
    def generate_parameters(self, batch_shape: torch.Size) -> Iterator[Tuple[ParamItem, int]]:
        ...
        if not self.same_on_batch and self.random_apply:
            ...
            for i in range(batch_shape[0]):
                ...
                with_mix = mix_added
                for s in seq:
                    if isinstance(s[1], (AugmentationBase, SequentialBase, K.MixAugmentationBaseV2)):
            ...
            for i, nchild in enumerate(self.named_children()):
                if isinstance(nchild[1], (AugmentationBase, SequentialBase, K.MixAugmentationBaseV2)):
