...
class ImageSequential(ImageSequentialBase, ImageModuleForSequentialMixIn):
    ...
    def _read_random_apply(
        ...
        """Read and validate the random_apply argument."""
    ...
    def get_random_forward_sequence(self, with_mix: bool = True) -> Tuple[Iterator[Tuple[str, Module]], bool]:
        ...
        # Mix augmentation can be applied once per forward
        mix_indices = self.get_mix_augmentation_indices(self.named_children())

        # -----------------------------------------------------------
        # FIX: Handle case where ONLY MixUp/CutMix exist
        # -----------------------------------------------------------
        if with_mix and len(mix_indices) == len(self):
            idx = torch.randint(0, len(self), (1,))
            return self.get_children_by_indices(idx), True
        # -----------------------------------------------------------

        # Remove mix ops from multinomial sampling
