    ...
    def compute_padding(self, shape: torch.Size) -> List[int]:
        ...
        if self.flags["padding"] is not None:
            if isinstance(self.flags["padding"], int):
                ...
                raise RuntimeError(
                    f"Expect `padding` to be a scalar, or length 2/4 list. Got {self.flags['padding']}.")
...
class RandomElasticTransform(GeometricAugmentationBase2D):
    ...
    def __init__(
        ...
        super().__init__(p=p, return_transform=return_transform, same_on_batch=same_on_batch, p_batch=1.0, keepdim=False)
        self.flags = dict(
            kernel_size=kernel_size,
            sigma=sigma,
            alpha=alpha,
    ...
    def apply_transform(
        ...
        return elastic_transform2d(
            input, params["noise"].to(input), self.flags["kernel_size"], self.flags["sigma"],
            self.flags["alpha"], self.flags["align_corners"], self.flags["mode"], self.flags["padding_mode"]
        )
...
class PadTo(GeometricAugmentationBase2D):
    ...
    def __init__(
        ...
        super().__init__(p=1., return_transform=return_transform, same_on_batch=True, p_batch=1., keepdim=keepdim)
        self.flags = dict(
            size=size,
            pad_mode=pad_mode,
            pad_value=pad_value,
