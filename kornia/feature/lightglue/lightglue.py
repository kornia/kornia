...
def rotate_half(x: torch.Tensor) -> torch.Tensor:
    ...
    x_shape = [i for i in x_orig_shape]
    x_shape[-1] //= 2
    x_shape.append(2)
    x = x.reshape(x_shape)  #  rearrange(x, '... (d r) -> ... d r', r=2)
    x1, x2 = x.unbind(dim=-1)
    x = torch.stack((-x2, x1), dim=-1)
    return x.reshape(x_orig_shape)  #  rearrange(x, '... d r -> ... (d r)')
...
class LearnableFourierPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return emb.repeat_interleave(2, dim=-1)  #  repeat(emb, '... n -> ... (n r)', r=2)
...
class Transformer(nn.Module):
    ...
    def _forward(self, x: torch.Tensor,
        ...
        qkv = qkv.reshape(b, n, h, -1, 3).permute(0, 2, 1, 3, 4)
        q, k, v = qkv[..., 0], qkv[..., 1], qkv[..., 2]
        if encoding is not None:
            q = apply_cached_rotary_emb(encoding, q)
            k = apply_cached_rotary_emb(encoding, k)
        context = self.inner_attn(q, k, v)
        b, h, n, d = context.shape
        message = self.out_proj(context.permute(0, 2, 1, 3).reshape(b, n, h * d))  # rearrange(context, 'b h n d -> b n (h d)'))
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, x1: torch.Tensor) -> List[torch.Tensor]:
        ...
        qk0, qk1, v0, v1 = map(
            lambda t:  t.reshape(t.shape[0], t.shape[1], h, t.shape[2] // h).permute(0, 2, 1, 3), #rearrange(t, 'b n (h d) -> b h n d', h=self.heads),
            (qk0, qk1, v0, v1))
        ...
        m0, m1 =self.map_(lambda t: t.permute(0, 2, 1, 3).reshape(t.shape[0], t.shape[2], -1),
                           m0, m1)
...
class LightGlue(nn.Module):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        kpts0 = normalize_keypoints(
            kpts0_, size=data.get('image_size0'))
        kpts1 = normalize_keypoints(
            kpts1_, size=data.get('image_size1'))
    ...
    def conf_th(self, i: int) -> float:
        ...
        return math_clamp(
            0.8 + 0.1 * math.exp(-4.0 * float(i) / self.conf.n_layers), 0., 1.)
