...
class Transformer(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, encoding: Optional[torch.Tensor] = None):
        ...
        message = self.out_proj(
            context.permute(0, 2, 1, 3).reshape(b, n, h * d)
        )  # rearrange(context, 'b h n d -> b n (h d)'))
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, x1: torch.Tensor) -> List[torch.Tensor]:
        ...
        qk0, qk1, v0, v1 = map(
            lambda t: t.reshape(t.shape[0], t.shape[1], h, t.shape[2] // h).permute(
                0, 2, 1, 3
        ...
        m0, m1 = self.map_(lambda t: t.permute(0, 2, 1, 3).reshape(t.shape[0], t.shape[2], -1), m0, m1)
...
class LightGlue(nn.Module):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        kpts0 = normalize_keypoints(kpts0_, size=data.get('image_size0'))
        kpts1 = normalize_keypoints(kpts1_, size=data.get('image_size1'))
