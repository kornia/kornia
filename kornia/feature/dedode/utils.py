...
@torch.no_grad()
def sample_keypoints(  # noqa: D103
...
def get_grid(B: int, H: int, W: int, device: torch.device) -> torch.Tensor:  # noqa: D103
...
def dedode_denormalize_pixel_coordinates(flow: torch.Tensor, h: int, w: int) -> torch.Tensor:  # noqa: D103
    flow = torch.stack(
        (
            w * (flow[..., 0] + 1) / 2,
            h * (flow[..., 1] + 1) / 2,
