...
class DeFMO(nn.Module):
    ...
    def __init__(self, pretrained: bool = False) -> None:
        ...
        if pretrained:
            pretrained_dict = torch.hub.load_state_dict_from_url(urls['defmo_encoder'], map_location=lambda storage, loc: storage)
            self.encoder.load_state_dict(pretrained_dict, strict=True)
            pretrained_dict_ren = torch.hub.load_state_dict_from_url(urls['defmo_rendering'], map_location=lambda storage, loc: storage)
