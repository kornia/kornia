...
from kornia.core import  pad, stack, zeros, IntegratedTensor
...
def get_motion_kernel2d(
    kernel_size: int, angle: IntegratedTensor | float, direction: IntegratedTensor | float = 0.0, mode: str = 'nearest'
) -> IntegratedTensor:
    ...
    device, dtype = _extract_device_dtype(
        [angle if isinstance(angle, IntegratedTensor) else None, direction if isinstance(direction, IntegratedTensor) else None]
    )
    ...
    if not isinstance(angle, IntegratedTensor):
        angle = IntegratedTensor([angle], dtype=dtype)

    if keras.ops.shape(angle) == 0:
        angle = angle[None]

    KORNIA_CHECK_SHAPE(angle, ['B'])

    if not isinstance(direction, IntegratedTensor):
        direction = IntegratedTensor([direction], dtype=dtype)

    if keras.ops.shape(direction) == 0:
        direction = direction[None]

    KORNIA_CHECK_SHAPE(direction, ['B'])
    KORNIA_CHECK(
        keras.ops.shape(direction)[0] == keras.ops.shape(angle)[0],
        f'direction and angle must have the same length. Got {direction} and {angle}.',
    )

    # direction from [-1, 1] to [0, 1] range
    direction = (keras.ops.clip(direction, -1.0, 1.0) + 1.0) / 2.0
    ...
    expected_shape = keras.ops.shape([keras.ops.shape(direction)[0], *kernel_tuple])
...
def get_motion_kernel3d(
    kernel_size: int, angle: IntegratedTensor | tuple[float, float, float], direction: IntegratedTensor | float = 0.0, mode: str = 'nearest'
) -> IntegratedTensor:
    ...
    # direction from [-1, 1] to [0, 1] range
    direction = (keras.ops.clip(direction, -1.0, 1.0) + 1.0) / 2.0
    kernel = zeros((keras.ops.shape(direction)[0], *kernel_tuple), dtype=dtype)
