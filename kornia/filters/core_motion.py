...
class MotionBlur(Module):
    ...
    def forward(self, x: IntegratedTensor) -> IntegratedTensor:
...
class MotionBlur3D(Module):
    ...
    def __init__(
        self,
        kernel_size: int,
        angle: float | tuple[float, float, float] | IntegratedTensor,
        direction: float | IntegratedTensor,
        ...
        self.kernel_size = kernel_size
        KORNIA_CHECK(
            isinstance(angle, (IntegratedTensor, float, list, tuple)),
    ...
    def call(self, x: IntegratedTensor) -> IntegratedTensor:
...
def motion_blur3d(
    input: IntegratedTensor,
    kernel_size: int,
    angle: tuple[float, float, float] | IntegratedTensor,
    direction: float | IntegratedTensor,
    border_type: str = 'constant',
    mode: str = 'nearest',
) -> IntegratedTensor:
