...
from kornia.contrib.models.efficient_vit.nn.ops import IdentityLayer, ResidualBlock
...
def apply_drop_func(network: nn.Module, drop_config: dict[str, any] or None) -> None:
    ...
    drop_lookup_table = {"droppath": apply_droppath}
...
def apply_droppath(network: nn.Module, drop_prob: float, linear_decay=True, scheduled=True) -> None:
    n_valid_blocks = 0
        ...
        for name, sub_module in m.named_children():
            if isinstance(sub_module, ResidualBlock) and isinstance(sub_module.shortcut, IdentityLayer):
                ...
                to_update_dict[name] = DropPathResidualBlock(
                    sub_module.main, sub_module.shortcut, sub_module.post_act, sub_module.pre_norm, prob, scheduled
                )
                _id += 1
