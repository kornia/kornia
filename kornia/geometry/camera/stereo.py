...
class StereoCamera:
    ...
    Convention:
        ...
        - a point cloud is a homogeneous transform by :attr:`Q` followed by the divide by ``W``, so the sign
          of :attr:`Q` cancels: :attr:`Q` and its negation return the same points. The matrix written on
          :doc:`/geometry.camera.stereo` is this one negated.
    ...
    .. warning::
        Three constructor guards do not enforce the contract above. A differing ``cx`` is **rejected**, even
        ...
        an empty tensor is ``True`` (`#4281 <https://github.com/kornia/kornia/issues/4281>`_). Tracked as
