...
def resize(
    ...
    Args:
        ...
        align_corners: interpolation flag.
        side: which side to resize when size is an int. One of ``'short'``, ``'long'``, ``'vert'``, ``'horz'``.
            Defaults to ``'short'``.
...
def resize_to_be_divisible(
    ...
    """
    if isinstance(input, torch.Tensor) and len(input.shape) == 4:
        height, width = input.shape[2], input.shape[3]
    if isinstance(input, torch.Tensor) and len(input.shape) == 3:
