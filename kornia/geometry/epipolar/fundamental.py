...
def normalize_points(points: torch.Tensor, eps: float = 1e-8) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Returns:
       ...
       shape (B, N, 2) and the transformation matrix in the shape (B, 3, 3).
...
def normalize_transformation(M: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: the normalized transformation matrix.
...
def find_fundamental(points1: torch.Tensor, points2: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: the computed fundamental matrix.
...
def compute_correspond_epilines(points: torch.Tensor, F_mat: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: a tensor with shape (B, N, 3) containing a vector of the epipolar lines
        corresponding to the points to the other image.
        Each line is described as `ax + by + c = 0` and encoding the vectors as (a, b, c).
...
def fundamental_from_essential(E_mat: torch.Tensor, K1: torch.Tensor, K2: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        E_mat: The essential matrix with shape of (*, 3, 3).
...
def fundamental_from_projections(P1: torch.Tensor, P2: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        P1: The projection matrix from first camera with shape (*, 3, 4).
        P2: The projection matrix from second camera with shape (*, 3, 4).
