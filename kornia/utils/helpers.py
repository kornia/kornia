...
def _torch_diff_where(
    x: Tensor,
    condition: Union[float, Tensor],
    a: Union[float, Tensor],
    b: Union[float, Tensor],
    alpha: Union[float, Tensor] = 1000,
) -> Tensor:
    """Helper function to implement a differentiable approximation of `torch.where(x < condition, a, b)` based on a
    sigmoid.
