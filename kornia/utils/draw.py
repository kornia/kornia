...
def draw_line(image: torch.Tensor, p1: torch.Tensor, p2: torch.Tensor, color: torch.Tensor, width: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        width: the width of the line with shape (1) or (B, 1).
    Return:
    ...
    if width.size(0) != 1 and width.size(0) != image.size(0):
        raise ValueError("width must have the same batch size as that of image or 1.")
