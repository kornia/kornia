...
def rgb_to_yuv(rgb: Tensor) -> Tensor:
    ...
    yuv = TF.conv2d(rgb, weights, bias=None)
...
def yuv_to_rgb(yuv: torch.Tensor) -> torch.Tensor:
    ...
    yuv, shape = reduce_first_dims(yuv, keep_last_dims=3)

    weights = _YUV2RGB_WEIGHTS.type_as(yuv)
    rgb = TF.conv2d(yuv, weights, bias=None)
...
def yuv420_to_rgb(y: torch.Tensor, uv: torch.Tensor) -> torch.Tensor:
    ...
    uv, uv_shape = reduce_first_dims(uv, keep_last_dims=3)

    KORNIA_CHECK_SHAPE(y, ["*", "1", str(2 * uv_shape[-2]), str(2 * uv_shape[-1])])
    y, y_shape = reduce_first_dims(y, keep_last_dims=3)
    ...
    rgb = TF.conv2d(yuv444image, weights, bias=None)
