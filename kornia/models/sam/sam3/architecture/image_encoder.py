...
from .common import Attention, LayerNorm, MLPBlock
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        if len(x.shape) != 4:
            raise ValueError(f"Expected 4D input tensor, got {len(x.shape)}D")
...
class ImageEncoderHiera(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if len(x.shape) != 4 or x.shape[1] != 3:
            raise ValueError(f"Expected (B, 3, H, W) input tensor, got {x.shape}")
