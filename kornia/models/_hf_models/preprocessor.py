...
class DPTImageProcessor(PreprocessingLoader):
    ...
    def from_json(json_data: dict[str, Any]) -> ImageSequential:
        ...
        if json_data["do_resize"]:
            ...
            preproc_list.append(PreprocessingLoader.resize(
                width=json_data["size"]["width"], height=json_data["size"]["height"]))
        if json_data["do_rescale"]:
            preproc_list.append(PreprocessingLoader.rescale(
                rescale_factor=json_data["rescale_factor"] * 255))
        if json_data["do_normalize"]:
            preproc_list.append(PreprocessingLoader.normalize(
