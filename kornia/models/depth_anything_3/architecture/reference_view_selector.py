...
def select_reference_view(
    ...
    if strategy == "saddle_balanced":
        ...
        balance_score = (sim_score_norm - 0.5).abs() + (norm_norm - 0.5).abs() + (var_norm - 0.5).abs()
...
def reorder_by_reference(
    ...
    reorder_indices = torch.where((positions > 0) & (positions <= b_idx_expanded), positions - 1, positions)
...
def restore_original_order(
    ...
    restore_indices = torch.scatter(restore_indices, dim=1, index=b_idx_expanded, src=torch.zeros_like(b_idx_expanded))
