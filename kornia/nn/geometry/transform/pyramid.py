...
class PyrDown(nn.Module):
    ...
    def __init__(self, border_type: str = 'reflect', align_corners: bool = False) -> None:
        ...
        self.kernel: torch.Tensor = kornia.geometry.transform.pyramid._get_pyramid_gaussian_kernel()
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        if not len(input.shape) == 4:
            raise ValueError("Invalid input shape, we expect BxCxHxW. Got: {}"
                             .format(input.shape))
        # blur image
        x_blur: torch.Tensor = kornia.filters.filter2D(input, self.kernel, self.border_type)
...
class PyrUp(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        x_blur: torch.Tensor = kornia.filters.filter2D(x_up, self.kernel, self.border_type)
        return x_blur
...
class ScalePyramid(nn.Module):
    ...
    def get_first_level(self, input):
        ...
        if self.init_sigma > cur_sigma:
            ...
            cur_level = kornia.filters.gaussian_blur2d(x, (ksize, ksize), (sigma, sigma))
    ...
    def forward(self, x: torch.Tensor) -> Tuple[  # type: ignore
        ...
        while True:
            ...
            for level_idx in range(1, self.n_levels + self.extra_levels):
                ...
                cur_level = kornia.filters.gaussian_blur2d(
