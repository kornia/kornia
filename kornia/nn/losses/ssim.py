...
class SSIM(nn.Module):
    ...
    def __init__(
        ...
        self.window: torch.Tensor = kornia.filters.get_gaussian_kernel2d(
    ...
    def forward(  # type: ignore
        ...
        mu1: torch.Tensor = kornia.filters.filter2D(img1, tmp_kernel)
        mu2: torch.Tensor = kornia.filters.filter2D(img2, tmp_kernel)
        ...
        sigma1_sq = kornia.filters.filter2D(img1 * img1, tmp_kernel) - mu1_sq
        sigma2_sq = kornia.filters.filter2D(img2 * img2, tmp_kernel) - mu2_sq
        sigma12 = kornia.filters.filter2D(img1 * img2, tmp_kernel) - mu1_mu2
