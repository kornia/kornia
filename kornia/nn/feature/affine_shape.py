...
class PatchAffineShapeEstimator(nn.Module):
    ...
    def __init__(self, patch_size: int = 19, eps: float = 1e-10):
        ...
        self.gradient: nn.Module = kornia.filters.SpatialGradient('sobel', 1)
        self.eps: float = eps
        sigma: float = float(self.patch_size) / math.sqrt(2.0)
        self.weighting: torch.Tensor = kornia.filters.get_gaussian_kernel2d(
...
class LAFAffineShapeEstimator(nn.Module):
    ...
    def forward(self, laf: torch.Tensor, img: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        kornia.feature.laf.raise_error_if_laf_is_not_valid(laf)
        ...
        patches: torch.Tensor = kornia.feature.extract_patches_from_pyramid(
            img, kornia.feature.laf.make_upright(laf), PS, True).view(-1, 1, PS, PS)
        ...
        scale_orig = kornia.feature.laf.get_laf_scale(laf)
        laf_out = kornia.feature.laf.ellipse_to_laf(ellipses)
        ellipse_scale = kornia.feature.laf.get_laf_scale(laf_out)
        laf_out = kornia.feature.laf.scale_laf(laf_out, scale_orig / ellipse_scale)
        return laf_out
