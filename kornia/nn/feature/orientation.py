...
class PatchDominantGradientOrientation(nn.Module):
    ...
    def __init__(self,
        ...
        self.gradient = kornia.filters.SpatialGradient('sobel', 1)
        self.eps = eps
        ...
        self.weighting = kornia.filters.get_gaussian_kernel2d((self.patch_size, self.patch_size), (sigma, sigma), True)
        return
    ...
    def forward(self, patch: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        ori: torch.Tensor = torch.atan2(gy, gx + self.eps) + 2.0 * kornia.constants.pi

        o_big = float(self.num_ang_bins) * (ori + 1.0 * kornia.constants.pi) / (2.0 * kornia.constants.pi)
        ...
        angle = -((2. * kornia.constants.pi * indices.to(patch.dtype) / float(
            self.num_ang_bins)) - kornia.constants.pi)  # type: ignore
        return angle
...
class LAFOrienter(nn.Module):
    ...
    def forward(self, laf: torch.Tensor, img: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        kornia.feature.raise_error_if_laf_is_not_valid(laf)
        ...
        B, N = laf.shape[:2]
        patches: torch.Tensor = kornia.feature.extract_patches_from_pyramid(
            img, laf, self.patch_size).view(-1, 1, self.patch_size, self.patch_size)
        angles_radians: torch.Tensor = self.angle_detector(patches).view(B, N)
        rotmat: torch.Tensor = kornia.geometry.angle_to_rotation_matrix(
            kornia.geometry.rad2deg(angles_radians)).view(B * N, 2, 2)

        laf_out: torch.Tensor = torch.cat([
            torch.bmm(kornia.feature.make_upright(laf).view(B * N, 2, 3)[:, :2, :2], rotmat),
