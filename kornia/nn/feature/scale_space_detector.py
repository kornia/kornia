...
import kornia
from kornia.nn.feature.responses import BlobHessian
from kornia.nn.geometry.subpix import ConvSoftArgmax3d
from kornia.nn.feature.orientation import PassLAF
from kornia.nn.geometry.transform import ScalePyramid
...
class ScaleSpaceDetector(nn.Module):
    ...
    def detect(self, img: torch.Tensor, num_feats: int,
        ...
        for oct_idx, octave in enumerate(sp):
            ...
            if mask is not None:
                oct_mask: torch.Tensor = kornia.feature.scale_space_detector._create_octave_mask(mask, oct_resp.shape)
            ...
            max_coords_best = kornia.feature.scale_space_detector._scale_index_to_scale(
            ...
            good_mask = kornia.feature.laf.laf_is_inside_image(current_lafs, octave[:, 0])
            resp_flat_best = resp_flat_best * good_mask.to(dev, dtype)

            # Normalize LAFs
            current_lafs = kornia.feature.laf.normalize_laf(
                current_lafs, octave[:, 0])  # We don`t need # of scale levels, only shape
        ...
        return responses, kornia.feature.laf.denormalize_laf(lafs, img)
