...
def _build_registry():
    ...
    reg.append(
        ("VerticalFlip", "geometric", lambda: K.RandomVerticalFlip(p=1.0), lambda: T.RandomVerticalFlip(p=1.0), "image")
    )
    reg.append(
        (
            "Rotation",
            ...
            lambda: T.RandomRotation(degrees=15.0),
            "image",
        )
    )
    reg.append(
        (
            "Affine",
            "geometric",
            lambda: K.RandomAffine(degrees=15, translate=(0.1, 0.1), scale=(0.8, 1.2), p=1.0),
            lambda: T.RandomAffine(degrees=15, translate=(0.1, 0.1), scale=(0.8, 1.2)),
            "image",
        )
    )
    reg.append(
        (
            ...
            lambda: T.RandomResizedCrop(size=224),
            "image",
        )
    )
    reg.append(("CenterCrop", "geometric", lambda: K.CenterCrop(size=224), lambda: T.CenterCrop(size=224), "image"))
    reg.append(("Resize", "geometric", lambda: K.Resize(size=224), lambda: T.Resize(size=224), "image"))
    reg.append(
        (
            "Perspective",
            "geometric",
            lambda: K.RandomPerspective(distortion_scale=0.2, p=1.0),
            lambda: T.RandomPerspective(distortion_scale=0.2, p=1.0),
            "image",
        )
    )
    ...
    reg.append(
        (
            ...
            lambda: T.ColorJitter(brightness=0.2),
            "image",
        )
    )
    reg.append(
        (
            ...
            lambda: T.ColorJitter(contrast=0.2),
            "image",
        )
    )
    reg.append(
        (
            ...
            lambda: T.ColorJitter(saturation=0.2),
            "image",
        )
    )
    reg.append(
        ("Hue", "intensity_color", lambda: K.RandomHue(hue=(-0.1, 0.1), p=1.0), lambda: T.ColorJitter(hue=0.1), "image")
    )
    reg.append(
        ("Grayscale", "intensity_color", lambda: K.RandomGrayscale(p=1.0), lambda: T.RandomGrayscale(p=1.0), "image")
    )
    reg.append(
        (
            "Solarize",
    ...
    reg.append(
        ("Equalize", "intensity_color", lambda: K.RandomEqualize(p=1.0), lambda: T.RandomEqualize(p=1.0), "image")
    )
    reg.append(("Invert", "intensity_color", lambda: K.RandomInvert(p=1.0), lambda: T.RandomInvert(p=1.0), "image"))
    reg.append(
        (
            "Sharpness",
            "intensity_color",
            lambda: K.RandomSharpness(sharpness=0.5, p=1.0),
            lambda: T.RandomAdjustSharpness(sharpness_factor=2.0, p=1.0),
            "image",
        )
    )
    ...
    reg.append(
        (
            "GaussianBlur",
            "intensity_blur",
            lambda: K.RandomGaussianBlur(kernel_size=(5, 5), sigma=(0.1, 2.0), p=1.0),
            lambda: T.GaussianBlur(kernel_size=5, sigma=(0.1, 2.0)),
            "image",
        )
    )
    reg.append(
        (
            ...
            lambda: T.GaussianNoise(sigma=0.05),
            "image",
        )
    )
    reg.append(
        (
            "MotionBlur",
            "intensity_blur",
            lambda: K.RandomMotionBlur(kernel_size=5, angle=35.0, direction=0.5, p=1.0),
            None,
            "image",
        )
    )
    reg.append(("BoxBlur", "intensity_blur", lambda: K.RandomBoxBlur(kernel_size=(3, 3), p=1.0), None, "image"))
    reg.append(("MedianBlur", "intensity_blur", lambda: K.RandomMedianBlur(kernel_size=(3, 3), p=1.0), None, "image"))

    # ERASING
    reg.append(("RandomErasing", "erasing", lambda: K.RandomErasing(p=1.0), lambda: T.RandomErasing(p=1.0), "image"))
    ...
    reg.append(("Denormalize", "normalize", lambda: K.Denormalize(mean=MEAN_T, std=STD_T), None, "image"))

    # MIX
    reg.append(("MixUp", "mix", lambda: K.RandomMixUpV2(p=1.0), lambda: T.MixUp(num_classes=1000), "labels"))
    reg.append(("CutMix", "mix", lambda: K.RandomCutMixV2(p=1.0), lambda: T.CutMix(num_classes=1000), "labels"))
    reg.append(("Mosaic", "mix", lambda: K.RandomMosaic(output_size=(512, 512), p=1.0), None, "image"))

    # KORNIA-ONLY
    reg.append(("RandomRain", "kornia_only", lambda: K.RandomRain(p=1.0), None, "image"))
    reg.append(("RandomSnow", "kornia_only", lambda: K.RandomSnow(p=1.0), None, "image"))
    reg.append(("RandomChannelDropout", "kornia_only", lambda: K.RandomChannelDropout(p=1.0), None, "image"))
    reg.append(("RandomChannelShuffle", "kornia_only", lambda: K.RandomChannelShuffle(p=1.0), None, "image"))
    reg.append(("RandomRGBShift", "kornia_only", lambda: K.RandomRGBShift(p=1.0), None, "image"))
    reg.append(("RandomPlanckianJitter", "kornia_only", lambda: K.RandomPlanckianJitter(p=1.0), None, "image"))
    reg.append(("RandomCLAHE", "kornia_only", lambda: K.RandomClahe(p=1.0), None, "image"))
...
def extract_top_ops(prof, n=15):
    ...
    sorted_avgs = sorted(avgs, key=lambda x: getattr(x, "self_cpu_time_total", 0), reverse=True)
    out = []
...
def aggregate_totals(prof, n_iters):
    ...
    copy_event_count = 0
    SYNC_OPS = {"aten::_local_scalar_dense", "aten::item", "aten::is_nonzero", "cudaStreamSynchronize"}
...
def categorize_op(name, k_top, k_totals):
    ...
    real_excl_copy = _top_excluding(k_top, {"aten::copy_"}, exclude_substr=("_kornia", "_tv"))
...
def write_categorized_md(out_path: Path, results: dict, versions: dict, per_op_medians: dict, patch_status: str):
    lines = []
    lines.append("# Bottleneck categorization — all 37 augmentation transforms")
    lines.append("")
    lines.append("**Date:** 2026-04-27  ")
    lines.append(f"**Hardware:** {versions.get('device', '?')} ({versions.get('machine', '?')})  ")
    lines.append(
    ...
    lines.append(
        f"**Profile:** {N_WARMUP} warmup + {N_ITERS} timed iters via "
    ...
    lines.append("")
    lines.append(
        "**CUPTI note:** `CUPTI_ERROR_INSUFFICIENT_PRIVILEGES` on this "
        ...
        rows.append(
            {
                ...
                "sync_per_iter": ((k_tot.get("sync_event_count", 0) / N_ITERS) if k_tot else 0),
            }
        )
    ...
    for c in cat_order:
        if c in cat_totals:
            t = cat_totals[c]
            lines.append(f"| {c} | {t['count']} | {_fmt(t['k_ms_sum'])} | {_fmt(t['tv_ms_sum'])} |")
    lines.append("")

    lines.append("## Per-op classification")
    lines.append("")
    lines.append(
        "| Op | k ms | tv ms | events/iter | self CPU ms | "
    ...
    rows_sorted = sorted(
        rows,
        key=lambda r: (cat_rank.get(r["category"], 99), -(r["k_ms"] or 0)),
    )
    ...
    def _by_k(rs):
        return sorted([r for r in rs if r["k_ms"] is not None], key=lambda r: -r["k_ms"])
    ...
    def _section(title, items, est_speedup, difficulty):
        ...
        for r in items[:15]:
            ...
            lines.append(f"  - **{r['name']}** (k={_fmt(r['k_ms'])} ms{ratio_s}) — {r['dominant']}")
        lines.append("")
    ...
    _section(
        ...
        "**high** (cross-cutting refactor of `_AugmentationBase`, parameter sampler, and forward dispatch)",
    )
    _section(
    ...
    _section(
        ...
        "**medium** (audit each call site; some are baked into augmentation control flow)",
    )
    _section(
    ...
    _section(
        ...
        "**high** (per-kernel rewrite; gains are smaller and hardware-specific)",
    )

    lines.append("## Concrete recommendations for kornia 2.0 RFC")
    lines.append("")
    lines.append("Based on the categorization above, these architectural decisions are justified:")
    lines.append("")
...
def main():
    ...
    for name, category, k_fac, tv_fac, kind in registry:
        try:
            prof_k, prof_tv, k_err, tv_err = profile_op(name, k_fac, tv_fac, kind)
            row = {
                "category": category,
    ...
    print(f"\nTotal elapsed: {elapsed:.1f}s ({n_ok}/{n_total} ops profiled, {n_err} errors)", flush=True)
