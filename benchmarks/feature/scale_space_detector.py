...
def make_baseline(num_features: int, device: torch.device) -> KF.ScaleSpaceDetector:
    ...
    subpix = ConvSoftArgmax3d(
        (3, 3, 3), (1, 1, 1), (1, 1, 1),
        normalized_coordinates=False, output_value=True,
    )
...
def eval_sequence(
    ...
    for k, img_path, h_path in pairs:
        ...
        rows.append({"k": k, "mae_b": mae_b, "mae_n": mae_n,
                     "ni_b": ni_b, "ni_n": ni_n, "ms_b": ms_b, "ms_n": ms_n})
...
def print_sequence_table(stats: dict) -> None:
    ...
    print(f"\n── {seq}  ({stats['h']}×{stats['w']}) ──")
    print(f"{'pair':<6} {'MAE_base':>10} {'MAE_new':>10} {'Δ MAE':>8} "
          f"{'inl_b':>7} {'inl_n':>7} {'ms_b':>8} {'ms_n':>8}")
    print("─" * 68)
    for r in rows:
        delta = r["mae_b"] - r["mae_n"]
        print(f"1↔{r['k']:<3}  {r['mae_b']:>10.2f} {r['mae_n']:>10.2f} {delta:>+8.2f} "
    ...
    print(f"{'mean':<6}  {mae_b:>10.2f} {mae_n:>10.2f} {mae_b-mae_n:>+8.2f} "
...
def print_summary(all_stats: list[dict]) -> None:
    ...
    print(f"  Mean MAE  baseline / new     : {all_mae_b:.2f} / {all_mae_n:.2f} px  "
          f"→ {all_mae_b/max(all_mae_n, 1e-9):.2f}× improvement")
    print(f"  Mean inliers  baseline / new : {all_ni_b:.1f} / {all_ni_n:.1f}  "
          f"→ {all_ni_n/max(all_ni_b, 1e-9):.2f}× improvement")
    print(f"  Mean time  baseline / new    : {all_ms_b:.1f} / {all_ms_n:.1f} ms  "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--seq",    metavar="DIR",  action="append", default=[],
                   help="Path to an Oxford-Affine sequence directory (repeatable).")
    p.add_argument("--root",   metavar="DIR",  default=None,
                   help="Root folder: every subdirectory is treated as a sequence.")
    p.add_argument("--nf",     metavar="N",    type=int, default=2000,
                   help="Number of features per image (default: 2000).")
    p.add_argument("--device", metavar="DEV",  default=None,
                   help="'cpu', 'cuda', or 'cuda:N' (default: cuda if available, else cpu).")
    p.add_argument("--warmup", metavar="N",    type=int, default=1,
                   help="Warmup passes on img1↔img2 before timing (default: 1).")
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"  baseline : ConvSoftArgmax3d  (dense soft-NMS)")
    print(f"  new      : AdaptiveQuadInterp3d  (sparse NMS + iterative subpixel)")
