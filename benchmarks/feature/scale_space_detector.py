...
def print_summary(all_stats, label_a: str, label_b: str) -> None:
    ...
    print(f"  MAE     A / B : {mae_a:.2f} / {mae_b:.2f} px" f"  -> {mae_a / max(mae_b, 1e-9):.2f}x  (>1 = B has lower error)")
    print(f"  inliers B / A : {ni_b:.1f} / {ni_a:.1f}" f"  -> {ni_b / max(ni_a, 1e-9):.2f}x  (>1 = B finds more inliers)")
    print(f"  time    A / B : {ms_a:.1f} / {ms_b:.1f} ms" f"  -> {ms_a / max(ms_b, 1e-9):.2f}x  (>1 = B is faster)")
...
def _add_side(p: argparse.ArgumentParser, side: str, default_method: str, default_subpix: str) -> None:
    ...
    g.add_argument(f"--{side}-method", dest=f"{side}_method", default=default_method, choices=["scalespace", "aliked", "disk", "dedode"])
    g.add_argument(
        f"--{side}-resp",
        dest=f"{side}_resp",
        default="dog",
...
def main() -> None:
    ...
    ext_a = build_extractor(args.a_method, args.a_resp, args.a_subpix, args.a_desc, args.a_ori, args.a_aff, device, args.nf)
    ext_b = build_extractor(args.b_method, args.b_resp, args.b_subpix, args.b_desc, args.b_ori, args.b_aff, device, args.nf)
