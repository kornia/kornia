...
def scale_space_environment(paths: list[Path], cpu_label: str | None = None) -> str:
    """Validate the fixed chart protocol and derive its hardware label."""
    metadata = [json.loads(path.read_text())["metadata"] for path in paths]
    if not metadata:
    ...
    for meta in metadata:
        for key, value in expected.items():
            if meta.get(key) != value:
                raise ValueError(f"Scale-space chart requires {key}={value!r}")
        if meta.get("device") not in ("cpu", "mps"):
            raise ValueError("Scale-space chart supports CPU/MPS batch-one measurements")
        if meta.get("ransac") != {
            "model": "homography",
            "seed": 3407,
            ...
            raise ValueError("Scale-space chart requires the fixed Graf RANSAC protocol")
    for key in ("input_sha256", "python", "torch", "platform", "machine", "ransac_corner_metric"):
        if metadata[0].get(key) is None or any(meta.get(key) != metadata[0][key] for meta in metadata):
            raise ValueError(f"Input metadata must agree on {key}")
    recorded = {meta["cpu_model"] for meta in metadata if meta.get("cpu_model")}
    if len(recorded) > 1:
        raise ValueError("Cannot combine different CPU models")
    return cpu_label or next(iter(recorded), f"{metadata[0]['machine']} (CPU model not recorded)")


def plot_scale_space(paths: list[Path], labels: list[str], output: Path, cpu_label: str | None = None) -> None:
    """Plot per-pair geometry and end-to-end timings from scale-space benchmark JSON."""
    import matplotlib.pyplot as plt
    import numpy as np

    if len(paths) != len(labels):
        raise ValueError("Provide one --labels entry per input JSON")
    environment = scale_space_environment(paths, cpu_label)
    measurements = {}
    for path, label in zip(paths, labels):
        ...
        if key in measurements:
            raise ValueError(f"Duplicate series/device {key}")
        rows = document["results"]
        quality = {r["pair"]: r for r in rows if r["op"] == "matching_quality"}
        timing = [r for r in rows if r["op"] in ("SIFTFeatureScaleSpace", "OpenCVSIFT")]
        if len(timing) != 6 or len(quality) != 5:
            raise ValueError(f"{path}: expected six image timings and five matching pairs")
        if set(quality) != {f"1-{i}" for i in range(2, 7)} or {r["image"] for r in timing} != set(range(1, 7)):
            raise ValueError(f"{path}: expected Graf images 1-6 and pairs 1-2 through 1-6")
        for row in timing:
            normalize_timing(row)
            if (row["batch"], row["height"], row["width"]) != (1, 640, 800):
                raise ValueError(f"{path}: expected batch-one 640x800 images")
        measurements[key] = (quality, timing)
    series = list(dict.fromkeys(labels))
    devices = list(dict.fromkeys(d for _, d in measurements))
    if "cpu" not in devices:
        raise ValueError("The geometry panels require CPU measurements")
    colors = ["#2477ad", "#ee8c24", "#269a45", "#9055ad"]
    fig, axes = plt.subplots(3, 1, figsize=(9, 11), layout="constrained")
    pairs = [f"1-{i}" for i in range(2, 7)]
    for i, label in enumerate(series):
        if (label, "cpu") not in measurements:
            continue
        quality, _ = measurements[label, "cpu"]
        for ax, field in zip(axes[:2], ("ransac_corner_error_px", "ransac_inliers")):
            values = [quality[p][field] if quality[p][field] is not None else float("nan") for p in pairs]
            ax.plot(pairs, values, marker=["o", "x", "s", "^"][i % 4], color=colors[i % 4], label=label)
    axes[0].set_title("Homography estimation on Oxford graf — DoG + RootSIFT")
    axes[0].set_ylabel("Mean L1 corner error [px] (log)")
    axes[0].set_yscale("log")
    axes[0].legend()
    axes[1].set_title("Homography RANSAC consensus — same matcher and estimator")
    ...
    for i, label in enumerate(series):
        ...
        for j, device in enumerate(devices):
            if (label, device) not in measurements:
                continue
            _, rows = measurements[label, device]
            value = np.mean([r["median_us"] for r in rows]) / 1000
            pos = x[j] - 0.4 + width * (i + 0.5)
            axes[2].bar(pos, value, width, color=colors[i % 4], label=label if not labeled else None)
            labeled = True
            axes[2].text(pos, value * 1.06, f"{value:.0f}", ha="center", fontsize=9)
    axes[2].set_title("SIFT extraction runtime — detection + orientation + RootSIFT")
    axes[2].set_ylabel("Mean per-image median [ms] (log)")
    ...
    fig.supxlabel(
        ...
        "Large errors retained. No CUDA/compiled measurements. See report for versions and feature counts.",
        fontsize=9,
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    for suffix in (".png", ".svg"):
...
def main() -> None:
    ...
    parser.add_argument("--scale-space", action="store_true", help="Plot quality and runtime from sift_scale_space.py")
    parser.add_argument("--labels", nargs="+", help="Series label for each scale-space JSON; repeat across devices")
    args = parser.parse_args()
    if args.scale_space:
        if not args.labels:
            parser.error("--scale-space requires --labels")
        plot_scale_space(args.inputs, args.labels, args.output, args.cpu_label)
        return
    try:
