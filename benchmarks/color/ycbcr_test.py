...
def test_ycbcr_to_rgb(benchmark, device, dtype, torch_optimizer, B, C, H, W):
    data = torch.rand(B, C, H, W, device=device, dtype=dtype)

    op = torch_optimizer(ycbcr_to_rgb)
...
@pytest.mark.parametrize("H", [128, 256, 512])
@pytest.mark.parametrize("W", [128, 256, 512])
def test_rgb_to_ycbcr(benchmark, device, dtype, torch_optimizer, B, C, H, W):
    data = torch.rand(B, C, H, W, device=device, dtype=dtype)

    op = torch_optimizer(rgb_to_ycbcr)
