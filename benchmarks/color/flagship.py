...
loop, their native regime, except Lab, which OpenCV computes on float32 input so its output is
real CIE Lab rather than the 8-bit rescaling; scikit-image runs normalized float HWC images per
image. Output conventions differ and are not normalized away: OpenCV's YCrCb swaps the chroma
channels, 8-bit HSV and YCbCr are rescaled to 0-255, scikit-image's ``rgb2ycbcr`` uses the
16-235 studio range, and kornia's hue is in radians. OpenCV's Bayer pattern names are shifted by
one pixel relative to kornia's ``CFA`` names; the ``raw_to_rgb`` row times bilinear demosaicing
of the same single-channel mosaic in both. These are throughput comparisons, not
output-equivalent ones. Throughput is img/s.
...
def build_ops(
    ...
    def pil_loop(mode: str) -> Backend:
        return (lambda: [pil.fromarray(im).convert(mode) for im in imgs_u8]) if pil else None
...
def main() -> None:
    ...
    backends = ["kornia (eager)", "kornia (compiled)", "torchvision v2", "scikit-image", "opencv", "PIL"]
