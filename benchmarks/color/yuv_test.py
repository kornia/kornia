...
def test_rgb_to_yuv_benchmark(benchmark, device, dtype, torch_optimizer, B, C, H, W):
    """Benchmark the rgb_to_yuv function."""

    data = torch.rand(B, C, H, W, device=device, dtype=dtype)
    op = torch_optimizer(rgb_to_yuv)
...
@pytest.mark.parametrize("H", [128, 256, 512])
@pytest.mark.parametrize("W", [128, 256, 512])
def test_yuv_to_rgb_benchmark(benchmark, device, dtype, torch_optimizer, B, C, H, W):
    """Benchmark the yuv_to_rgb function."""

    data = torch.rand(B, C, H, W, device=device, dtype=dtype)
    op = torch_optimizer(yuv_to_rgb)
