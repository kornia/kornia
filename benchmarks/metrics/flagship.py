...
"""Flagship metrics benchmark: kornia.metrics vs scikit-image and OpenCV.

Covers the two image-quality metrics and the segmentation metric an evaluation loop computes
most often:

============  =============================================================  ======================
kornia        scikit-image / OpenCV (float HWC per-image loop)               notes
============  =============================================================  ======================
``psnr``      ``metrics.peak_signal_noise_ratio(data_range=1)``;             —
              ``cv2.PSNR(ref, test, 1.0)``
...
is differentiable; scikit-image and OpenCV run single normalized float32 HWC images on CPU in a
Python loop, contiguous and built outside the timed call. OpenCV has no SSIM function.
...
neither library has a counterpart. Throughput is image pairs per second (``img/s``).
...
def build_ops(
    ...
    refs_c = [np.ascontiguousarray(im) for im in refs_np]
    tests_c = [np.ascontiguousarray(im) for im in tests_np]
    ...
    if include("psnr"):
        ...
        row["opencv"] = (lambda: [cv2.PSNR(r, t, 1.0) for r, t in zip(refs_c, tests_c)]) if cv2 else None
        ops["psnr"] = row
...
def main() -> None:
    ...
    meta = start_run(
        ...
        units="img/s",
        regimes=[
            "kornia: batched float BCHW; scikit-image/opencv: float32 HWC per-image loop (CPU); img/s = image pairs/s"
        ],
        missing=[
            (name, err)
            for lib, name, err in [(skm, "scikit-image", skm_error), (cv2, "opencv", cv2_error)]
            if lib is None
        ],
    )
    ...
    results = run_batch_sweep(
        batch_list(args),
        lambda b: build_ops(
            b, args.size, args.size, device, dtype, args.compile, skm, cv2, args.skip_compile_ops, args.ops
        ),
        backends,
