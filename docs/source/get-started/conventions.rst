...
.. list-table::
   ...
   * - ``warp_affine3d``, ``center_crop3d``, ``crop_by_transform_mat3d``
     - ``True``
   * - ``warp_perspective3d``, ``homography_warp3d``, ``affine3d``, ``rotate3d``,
       ``crop_and_resize3d``, ``crop_by_boxes3d``
     - ``False``
...
warp reproduces the input under either setting. The sampling grid of
:func:`kornia.geometry.transform.homography_warp3d` (and so of ``warp_perspective3d``) is
built in ``float32``, so a ``float64`` volume is reproduced only to about ``1e-6``.
