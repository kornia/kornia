...
def angle_axis_to_rotation_matrix_numpy(angle_axis):
    ...
    Args:
        angle_axis (numpy.ndarray): array of 3d vector of axis-angle rotations
                                    of size (1, 3).
...
def rotation_matrix_to_quaternion(rotation_matrix):
    if isinstance(rotation_matrix, np.ndarray):
        ...
        if not (len(rotation_matrix.shape) == 3 and  # noqa
                rotation_matrix.shape[1] == 4):
...
def rotation_matrix_to_quaternion_numpy(rotation_matrix):
    ...
    if trace >= 0.0:
        ...
        j = (i + 1) % 3
        k = (j + 1) % 3
        t = np.sqrt(rotation_matrix[i, i] - rotation_matrix[j, j] - # noqa
