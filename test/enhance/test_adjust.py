...
class TestAdjustSaturation:
    ...
    def test_saturation_one(self, device, dtype):
        data = torch.tensor([[[.5, .5], [.5, .5]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]]],
                            device=device,
                            dtype=dtype)  # 3x2x2
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(kornia.adjust_saturation, (img, 2.), raise_exception=True)
...
class TestAdjustHue:
    ...
    def test_gradcheck(self, device, dtype):
        ...
        img = torch.rand(batch_size, channels, height, width, device=device, dtype=dtype)
        img = tensor_to_gradcheck_var(img)  # to var
        assert gradcheck(kornia.adjust_hue, (img, 2.), raise_exception=True)


class TestAdjustGamma:

    def test_gamma_zero(self, device, dtype):
        data = torch.tensor([[[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]]],
                            device=device,
                            dtype=dtype)  # 3x2x2
    ...
    def test_gamma_one_gain_two(self, device, dtype):
        ...
        expected = torch.tensor([[[1., 1.], [1., 1.]], [[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]]],
                                device=device,
                                dtype=dtype)  # 3x2x2
    ...
    def test_gamma_two(self, device, dtype):
        ...
        expected = torch.tensor([[[1., 1.], [1., 1.]], [[.25, .25], [.25, .25]], [[.0625, .0625], [.0625, .0625]]],
                                device=device,
                                dtype=dtype)  # 3x2x2
    ...
    def test_gamma_two_batch(self, device, dtype):
        ...
        expected = torch.tensor([[[[1., 1.], [1., 1.]], [[.25, .25], [.25, .25]], [[.0625, .0625], [.0625, .0625]]],
                                 [[[1., 1.], [1., 1.]], [[.25, .25], [.25, .25]], [[.0625, .0625], [.0625, .0625]]]],
                                device=device,
                                dtype=dtype)  # 2x3x2x2
    ...
    def test_gradcheck(self, device, dtype):
        ...
        img = tensor_to_gradcheck_var(img)  # to var
        assert gradcheck(kornia.adjust_gamma, (img, 1., 2.), raise_exception=True)
...
class TestAdjustContrast:
    ...
    def test_factor_tensor(self, device, dtype):
        # prepare input data
        data = torch.tensor(
            [[[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]], [[.5, .5], [.5, .5]]],
            device=device,
            dtype=dtype
        )  # 4x2x2

        expected = torch.tensor(
            [[[0., 0.], [0., 0.]], [[.5, .5], [.5, .5]], [[.375, .375], [.375, .375]], [[1., 1.], [1., 1.]]],
            device=device,
            dtype=dtype
        )  # 4x2x2
    ...
    def test_factor_tensor_color(self, device, dtype):
        ...
        expected = torch.tensor([[[[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]]],
    ...
    def test_factor_tensor_shape(self, device, dtype):
        # prepare input data
        data = torch.tensor(
            [[[[1., 1., .5], [1., 1., .5]], [[.5, .5, .25], [.5, .5, .25]], [[.25, .25, .25], [.6, .6, .3]]],
             [[[0., 0., 1.], [0., 0., .25]], [[.3, .3, .4], [.3, .3, .4]], [[.6, .6, 0.], [.3, .2, .1]]]],
            device=device,
            dtype=dtype
        )  # 2x3x2x3

        expected = torch.tensor(
            [[[[1., 1., .75], [1., 1., .75]], [[.75, .75, .375], [.75, .75, .375]], [[.375, .375, .375], [.9, .9, .45]]
              ], [[[0., 0., 1.], [0., 0., .5]], [[.6, .6, .8], [.6, .6, .8]], [[1., 1., 0.], [.6, .4, .2]]]],
            device=device,
            dtype=dtype
        )  # 2x3x2x3
    ...
    def test_gradcheck(self, device, dtype):
        ...
        img = torch.ones(batch_size, channels, height, width, device=device, dtype=dtype)
        img = tensor_to_gradcheck_var(img)  # to var
        assert gradcheck(kornia.adjust_contrast, (img, 2.), raise_exception=True)
...
class TestAdjustBrightness:
    ...
    def test_factor_minus(self, device, dtype):
        # prepare input data
        data = torch.tensor([[[1., 1.], [1., 1.]], [[.75, .75], [.75, .75]], [[.25, .25], [.25, .25]]],
                            device=device,
                            dtype=dtype)  # 3x2x2

        expected = torch.tensor([[[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]], [[0., 0.], [0., 0.]]],
                                device=device,
                                dtype=dtype)  # 3x2x2
    ...
    def test_factor_tensor_color(self, device, dtype):
        ...
        expected = torch.tensor([[[[1., 1.], [1., 1.]], [[.75, .75], [.75, .75]], [[.5, .5], [.5, .5]]],
    ...
    def test_gradcheck(self, device, dtype):
        ...
        img = torch.ones(batch_size, channels, height, width, device=device, dtype=dtype)
        img = tensor_to_gradcheck_var(img)  # to var
        assert gradcheck(kornia.adjust_brightness, (img, 2.), raise_exception=True)
...
class TestSharpness(BaseTester):
    ...
    @pytest.mark.parametrize(
        "batch_size, height, width, factor", [(1, 4, 5, 0.0), (1, 4, 5, 0.8), (2, 4, 5, 0.8),
    ...
    def test_value(self, device, dtype):
        ...
        # np.array(en) / 255.
        expected = torch.tensor([[[[0.4963, 0.7682, 0.0885], [0.1320, 0.3305, 0.6341], [0.4901, 0.8964, 0.4556]]]],
                                device=device,
                                dtype=dtype)
    ...
    def test_value_batch(self, device, dtype):
        ...
        # en = ImageEnhance.Sharpness(img).enhance(0.8)
        # np.array(en) / 255.
        expected_08 = torch.tensor([[[[0.4963, 0.7682, 0.0885], [0.1320, 0.3305, 0.6341], [0.4901, 0.8964, 0.4556]]],
                                    [[[0.6323, 0.3489, 0.4017], [0.0223, 0.2052, 0.2939], [0.5185, 0.6977, 0.8000]]]],
                                   device=device,
                                   dtype=dtype)

        expected_08_13 = torch.tensor([[[[0.4963, 0.7682, 0.0885], [0.1320, 0.3305, 0.6341], [0.4901, 0.8964, 0.4556]]],
...
class TestSolarize(BaseTester):
    ...
    @pytest.mark.parametrize(
        ...
        [(1, 4, 5, 0.8, None), (1, 4, 5, 0.8, 0.4), (2, 4, 5, 0.8, None), (1, 4, 5, torch.tensor(0.8), None),
         (2, 4, 5, torch.tensor(0.8), None), (2, 4, 5, torch.tensor([0.8, 0.7]), None),
    ...
    def test_value(self, device, dtype):
        ...
        # np.array(en) / 255.
        expected = torch.tensor([[[[0.49411765, 0.23529412, 0.08627451], [0.12941176, 0.30588235, 0.36862745],
...
class TestPosterize(BaseTester):
    ...
    @pytest.mark.parametrize(
        "batch_size, height, width, bits", [(1, 4, 5, 8), (2, 4, 5, 1), (2, 4, 5, 0), (1, 4, 5, torch.tensor(8)),
                                            (2, 4, 5, torch.tensor(8)), (2, 4, 5, torch.tensor([0, 8])),
    ...
    def test_value(self, device, dtype):
        ...
        # np.array(en) / 255.
        expected = torch.tensor([[[[0., 0.50196078, 0.], [0., 0., 0.50196078], [0., 0.50196078, 0.]]]],
                                device=device,
                                dtype=dtype)
