...
class TestAdjustSaturation:
    ...
    def test_saturation_one(self, device, dtype):
        data = torch.tensor(
            [[[.5, .5], [.5, .5]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]]], device=device, dtype=dtype
        )  # 3x2x2
...
class TestAdjustGamma:
    ...
    def test_gamma_zero(self, device, dtype):
        data = torch.tensor(
            [[[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]]], device=device, dtype=dtype
        )  # 3x2x2
    ...
    def test_gamma_one_gain_two(self, device, dtype):
        ...
        expected = torch.tensor(
            [[[1., 1.], [1., 1.]], [[1., 1.], [1., 1.]], [[.5, .5], [.5, .5]]], device=device, dtype=dtype
        )  # 3x2x2
...
class TestAdjustContrast:
    ...
    def test_factor_tensor_shape(self, device, dtype):
        ...
        expected = torch.tensor(
            [
                [
                    [[1., 1., .75], [1., 1., .75]], [[.75, .75, .375], [.75, .75, .375]],
                    [[.375, .375, .375], [.9, .9, .45]]
...
class TestAdjustBrightness:
    ...
    def test_factor_minus(self, device, dtype):
        # prepare input data
        data = torch.tensor(
            [[[1., 1.], [1., 1.]], [[.75, .75], [.75, .75]], [[.25, .25], [.25, .25]]], device=device, dtype=dtype
        )  # 3x2x2

        expected = torch.tensor(
            [[[.5, .5], [.5, .5]], [[.25, .25], [.25, .25]], [[0., 0.], [0., 0.]]], device=device, dtype=dtype
        )  # 3x2x2
...
class TestSharpness(BaseTester):
    ...
    @pytest.mark.parametrize(
        "batch_size, height, width, factor", [
            (1, 4, 5, 0.0), (1, 4, 5, 0.8), (2, 4, 5, 0.8), (1, 4, 5, torch.tensor(0.8)), (2, 4, 5, torch.tensor(0.8)),
    ...
    def test_value(self, device, dtype):
        ...
        expected = torch.tensor(
            [
                [
                    [
                        [0.49411765, 0.23529412, 0.08627451], [0.12941176, 0.30588235, 0.36862745],
...
class TestPosterize(BaseTester):
    ...
    @pytest.mark.parametrize(
        "batch_size, height, width, bits", [
            (1, 4, 5, 8), (2, 4, 5, 1), (2, 4, 5, 0), (1, 4, 5, torch.tensor(8)), (2, 4, 5, torch.tensor(8)),
            (2, 4, 5, torch.tensor([0, 8])), (3, 4, 5, torch.tensor([0, 1, 8]))
        ]
    )
    @pytest.mark.parametrize("channels", [1, 3, 5])
    ...
    def test_value(self, device, dtype):
        ...
        # np.array(en) / 255.
        expected = torch.tensor(
            [[[[0., 0.50196078, 0.], [0., 0., 0.50196078], [0., 0.50196078, 0.]]]], device=device, dtype=dtype
        )
