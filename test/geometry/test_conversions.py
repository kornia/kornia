...
class TestAngleAxisToQuaternion:
    ...
    def test_small_angle_z(self, device, dtype, atol, rtol):
        theta = 1.e-2
        angle_axis = torch.tensor((0., 0., theta), device=device, dtype=dtype)
        expected = torch.tensor((np.cos(theta / 2.), 0., 0., np.sin(theta / 2.)), device=device, dtype=dtype)
        quaternion = kornia.angle_axis_to_quaternion(angle_axis, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(quaternion, expected, atol=atol, rtol=rtol)
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.angle_axis_to_quaternion, order=QuaternionCoeffOrder.XYZW), (angle_axis, ),
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.angle_axis_to_quaternion, order=QuaternionCoeffOrder.WXYZ), (angle_axis, ),
            raise_exception=True
        )
...
class TestQuaternionToAngleAxis:
    ...
    def test_small_angle_y(self, device, dtype, atol, rtol):
        theta = 1.e-2
        quaternion = torch.tensor((np.cos(theta / 2), 0., np.sin(theta / 2), 0.), device=device, dtype=dtype)
        expected = torch.tensor((0., theta, 0.), device=device, dtype=dtype)
        angle_axis = kornia.quaternion_to_angle_axis(quaternion, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(angle_axis, expected, atol=atol, rtol=rtol)
    ...
    def test_small_angle_z(self, device, dtype, atol, rtol):
        theta = 1.e-2
        quaternion = torch.tensor((np.cos(theta / 2), 0., 0., np.sin(theta / 2)), device=device, dtype=dtype)
        expected = torch.tensor((0., 0., theta), device=device, dtype=dtype)
        angle_axis = kornia.quaternion_to_angle_axis(quaternion, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(angle_axis, expected, atol=atol, rtol=rtol)
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.quaternion_to_angle_axis, order=QuaternionCoeffOrder.XYZW), (quaternion, ),
                raise_exception=True
            )
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.quaternion_to_angle_axis, order=QuaternionCoeffOrder.WXYZ), (quaternion, ),
            raise_exception=True
        )
...
class TestRotationMatrixToQuaternion:
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.rotation_matrix_to_quaternion, eps=eps, order=QuaternionCoeffOrder.XYZW), (matrix, ),
                raise_exception=True
            )
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.rotation_matrix_to_quaternion, eps=eps, order=QuaternionCoeffOrder.WXYZ), (matrix, ),
            raise_exception=True
        )
...
class TestQuaternionToRotationMatrix:
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.quaternion_to_rotation_matrix, order=QuaternionCoeffOrder.XYZW), (quaternion, ),
                raise_exception=True
            )
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.quaternion_to_rotation_matrix, order=QuaternionCoeffOrder.WXYZ), (quaternion, ),
            raise_exception=True
        )
...
class TestQuaternionLogToExp:
    ...
    def test_pi_quaternion_x(self, device, dtype, atol, rtol):
        ...
        one = torch.tensor(1., device=device, dtype=dtype)
        quaternion_log = torch.tensor((1., 0., 0.), device=device, dtype=dtype)
        expected = torch.tensor((torch.cos(one), torch.sin(one), 0., 0.), device=device, dtype=dtype)
        quaternion_exp = kornia.quaternion_log_to_exp(quaternion_log, eps=eps, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(quaternion_exp, expected, atol=atol, rtol=rtol)
    ...
    def test_pi_quaternion_y(self, device, dtype, atol, rtol):
        ...
        one = torch.tensor(1., device=device, dtype=dtype)
        quaternion_log = torch.tensor((0., 1., 0.), device=device, dtype=dtype)
        expected = torch.tensor((torch.cos(one), 0., torch.sin(one), 0.), device=device, dtype=dtype)
        quaternion_exp = kornia.quaternion_log_to_exp(quaternion_log, eps=eps, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(quaternion_exp, expected, atol=atol, rtol=rtol)
    ...
    def test_pi_quaternion_z(self, device, dtype, atol, rtol):
        ...
        one = torch.tensor(1., device=device, dtype=dtype)
        quaternion_log = torch.tensor((0., 0., 1.), device=device, dtype=dtype)
        expected = torch.tensor((torch.cos(one), 0., 0., torch.sin(one)), device=device, dtype=dtype)
        quaternion_exp = kornia.quaternion_log_to_exp(quaternion_log, eps=eps, order=QuaternionCoeffOrder.WXYZ)
        assert_allclose(quaternion_exp, expected, atol=atol, rtol=rtol)
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.quaternion_log_to_exp, eps=eps, order=QuaternionCoeffOrder.XYZW), (quaternion, ),
                raise_exception=True
            )
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.quaternion_log_to_exp, eps=eps, order=QuaternionCoeffOrder.WXYZ), (quaternion, ),
            raise_exception=True
        )
...
class TestQuaternionExpToLog:
    ...
    def test_gradcheck_xyzw(self, device, dtype):
        ...
        with pytest.warns(UserWarning):
            assert gradcheck(
                partial(kornia.quaternion_exp_to_log, eps=eps, order=QuaternionCoeffOrder.XYZW), (quaternion, ),
                raise_exception=True
            )
    ...
    def test_gradcheck(self, device, dtype):
        ...
        # evaluate function gradient
        assert gradcheck(
            partial(kornia.quaternion_exp_to_log, eps=eps, order=QuaternionCoeffOrder.WXYZ), (quaternion, ),
            raise_exception=True
        )
...
class TestPolCartConversions:
    ...
    def test_pol2cart(self, batch_shape, device, dtype):
        ...
        assert gradcheck(
            kornia.pol2cart, (
                tensor_to_gradcheck_var(rho),
                tensor_to_gradcheck_var(phi),
            ), raise_exception=True
        )
    ...
    def test_cart2pol(self, batch_shape, device, dtype):
        ...
        assert gradcheck(
            kornia.cart2pol, (
                tensor_to_gradcheck_var(x),
                tensor_to_gradcheck_var(y),
            ), raise_exception=True
        )
...
class TestNormalizePixelCoordinates:
    ...
    def test_tensor_bhw2(self, device, dtype, atol, rtol):
        ...
        expected = kornia.utils.create_meshgrid(height, width, normalized_coordinates=True,
...
class TestDenormalizePixelCoordinates:
    ...
    def test_tensor_bhw2(self, device, dtype):
        ...
        expected = kornia.utils.create_meshgrid(height, width, normalized_coordinates=False,
                                                device=device).to(dtype=dtype)
