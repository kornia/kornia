...
class TestBoundingBoxInferring3D:
    ...
    def test_gradcheck(self, device, dtype):
        boxes = torch.tensor([[[0, 1, 2], [10, 1, 2], [10, 21, 2], [0, 21, 2], [0, 1, 32], [10, 1, 32], [10, 21, 32],
                               [0, 21, 32]]],
                             device=device,
                             dtype=dtype)
...
class TestCropAndResize3D:
    ...
    def test_crop(self, device, dtype):
        ...
        expected = torch.tensor([[[[[25.1667, 27.1667], [30.5000, 32.5000]], [[46.5000, 48.5000], [51.8333, 53.8333]]]]
                                 ],
                                device=device,
                                dtype=dtype)
    ...
    def test_crop_batch(self, device, dtype):
        ...
        depth, height, width = 2, 2, 2
        expected = torch.tensor([[[[[16.0000, 19.0000], [24.0000, 27.0000]], [[48.0000, 51.0000], [56.0000, 59.0000]]]],
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(kornia.crop_and_resize3d, (
            img,
            boxes,
            (4, 3, 2),
...
class TestCenterCrop3D:
    ...
    def test_center_crop_357(self, crop_size, device, dtype):
        ...
        expected = inp[:, :, (inp.size(2) // 2 - crop_size[0] // 2):(inp.size(2) // 2 + crop_size[0] // 2 + 1),
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(kornia.center_crop3d, (
            img,
            (3, 5, 7),
        ), raise_exception=True)
...
class TestCropByBoxes3D:
    ...
    def test_crop_by_boxes_resizing(self, device, dtype):
        ...
        expected = torch.tensor([[[[[57.0000, 59.0000], [71.0000, 73.0000]], [[106.0000, 108.0000],
                                                                              [120.0000, 122.0000]]]]],
                                device=device,
                                dtype=dtype)
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(
            kornia.geometry.transform.crop.crop_by_boxes3d, (
                inp,
                src_box,
                dst_box,
            ), raise_exception=True
