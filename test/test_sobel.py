...
class TestSobelEdges:
    ...
    def test_edges(self):
        ...
        edges = tgm.image.sobel_edges(inp)
    ...
    def test_gradcheck(self):
        ...
        assert gradcheck(tgm.image.sobel_edges, (img,), raise_exception=True)

    def test_jit(self):
        @torch.jit.script
        def op_script(input):
            return tgm.image.sobel_edges(input)
        img = torch.rand(2, 3, 4, 5)
        actual = op_script(img)
        expected = tgm.image.sobel_edges(img)
