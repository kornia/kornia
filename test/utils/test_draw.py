...
from kornia.utils import draw_convex_polygon, draw_rectangle
...
class TestFillConvexPolygon:
    def test_circle(self, device, dtype):
        ...
        inside = (((Y - ctr) ** 2 + (X - ctr) ** 2).sqrt() <= radius)[None, None].expand(b, c, h, w)
    ...
    def test_ellipse(self, device, dtype):
        ...
        inside = (((Y - ctr) ** 2 + ((X - ctr) / lam) ** 2).sqrt() <= radius)[None, None].expand(b, c, h, w)
