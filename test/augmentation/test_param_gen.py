...
class TestRandomProbGen:
    ...
    def test_color_jitter_gen(self):
        ...
        rng.manual_seed(42)
        batch_size = 8
        jitter_params = _random_color_jitter_gen(
            batch_size, brightness=0.2, contrast=0.3, saturation=0.4, hue=0.1, random_generator=rng)
        rng.manual_seed(42)
        jitter_params_tuple = _random_color_jitter_gen(
        ...
        assert set(list(jitter_params.keys())) == set(list(jitter_params_tuple.keys())) == \
            set(['brightness_factor', 'contrast_factor', 'hue_factor', 'saturation_factor', 'order']), \
            "Redundant keys found apart from %s" % str(expected_jitter_params.keys())

        assert_allclose(jitter_params['brightness_factor'], expected_jitter_params['brightness_factor'])
        assert_allclose(jitter_params['contrast_factor'], expected_jitter_params['contrast_factor'])
        assert_allclose(jitter_params['hue_factor'], expected_jitter_params['hue_factor'])
        assert_allclose(jitter_params['saturation_factor'], expected_jitter_params['saturation_factor'])
        assert_allclose(jitter_params['order'], expected_jitter_params['order'])

        assert_allclose(jitter_params_tuple['brightness_factor'], expected_jitter_params['brightness_factor'])
        assert_allclose(jitter_params_tuple['contrast_factor'], expected_jitter_params['contrast_factor'])
        assert_allclose(jitter_params_tuple['hue_factor'], expected_jitter_params['hue_factor'])
        assert_allclose(jitter_params_tuple['saturation_factor'], expected_jitter_params['saturation_factor'])
        assert_allclose(jitter_params_tuple['order'], expected_jitter_params['order'])
