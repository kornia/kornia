...
class TestRgbToYcbcr(BaseTester):
    ...
    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert kornia.color.rgb_to_ycbcr([0.])
    ...
    def test_unit(self, device, dtype):
        data = torch.tensor([[[[0.7925, 0.8704, 0.2771, 0.3279, 0.0193], [0.2483, 0.6212, 0.9859, 0.5044, 0.5621],
                               [0.5762, 0.3959, 0.2931, 0.2669, 0.0243], [0.6989, 0.0529, 0.8344, 0.6523, 0.8980],
                               [0.5181, 0.9341, 0.2172, 0.0520, 0.7266]],
                              [[0.8413, 0.0284, 0.3625, 0.8864, 0.5595], [0.3791, 0.0235, 0.4251, 0.0619, 0.5270],
                               [0.3516, 0.8005, 0.9571, 0.4113, 0.6119], [0.0632, 0.8836, 0.0261, 0.1550, 0.4923],
                               [0.2332, 0.7044, 0.9514, 0.2443, 0.2818]],
                              [[0.6899, 0.2063, 0.3179, 0.8989, 0.4378], [0.0384, 0.5230, 0.6416, 0.9749, 0.7863],
                               [0.8577, 0.3115, 0.2375, 0.5446, 0.9837], [0.3213, 0.6618, 0.5977, 0.3999, 0.4962],
...
class TestYcbcrToRgb(BaseTester):
    ...
    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert kornia.color.ycbcr_to_rgb([0.])
    ...
    def test_unit(self, device, dtype):
        data = torch.tensor([[[[0.9892, 0.2620, 0.4184, 0.5286, 0.2793], [0.0722, 0.8828, 0.8714, 0.0657, 0.7798],
                               [0.8118, 0.7522, 0.0260, 0.8811, 0.5226], [0.0644, 0.3648, 0.4448, 0.4202, 0.7316],
                               [0.9138, 0.1956, 0.4257, 0.6381, 0.1353]],
                              [[0.7408, 0.8529, 0.5119, 0.0220, 0.0226], [0.8963, 0.5652, 0.9568, 0.6977, 0.8221],
                               [0.4645, 0.0478, 0.4952, 0.5492, 0.4861], [0.9980, 0.9978, 0.0281, 0.5283, 0.8146],
                               [0.7789, 0.2663, 0.6437, 0.6926, 0.5627]],
                              [[0.7377, 0.7152, 0.3080, 0.8515, 0.4841], [0.7192, 0.3297, 0.7337, 0.0230, 0.2464],
                               [0.6399, 0.8998, 0.3838, 0.3043, 0.3774], [0.1281, 0.6731, 0.4218, 0.3963, 0.8541],
