...
class TestRgbToYuv(BaseTester):
    def test_smoke(self, device, dtype):
        ...
        assert isinstance(covolutional_color.rgb_to_yuv(img)[0], torch.Tensor)
        assert isinstance(covolutional_color.rgb_to_yuv(img)[1], torch.Tensor)

    @pytest.mark.parametrize("shape", [(1, 3, 4, 4), (2, 3, 2, 4), (3, 3, 4, 1), (3, 2, 1)])
    def test_cardinality(self, device, dtype, shape):
        img = torch.ones(shape, device=device, dtype=dtype)
        assert covolutional_color.rgb_to_yuv(img).shape == shape

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.rgb_to_yuv([0.0])

        with pytest.raises(TypeError):
            img = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.rgb_to_yuv(img)
    ...
    def test_unit(self, device, dtype):
        ...
        self.assert_close(color.rgb_to_yuv(data), covolutional_color.rgb_to_yuv(data))

    # TODO: improve accuracy
    def test_forth_and_back(self, device, dtype):
        data = torch.rand(3, 4, 5, device=device, dtype=dtype)
        yuv = covolutional_color.rgb_to_yuv
        rgb = covolutional_color.yuv_to_rgb
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(covolutional_color.rgb_to_yuv, (img,), raise_exception=True)
...
class TestRgbToYuv420(BaseTester):
    def test_smoke(self, device, dtype):
        B, C, H, W = 2, 3, 4, 6
        img = torch.rand(B, C, H, W, device=device, dtype=dtype)
        assert isinstance(covolutional_color.rgb_to_yuv420(img)[0], torch.Tensor)
        assert isinstance(covolutional_color.rgb_to_yuv420(img)[1], torch.Tensor)
    ...
    def test_cardinality(self, device, dtype, shape):
        ...
        shapeuv[-2] /= 2
        shapeuv[-1] /= 2
        assert covolutional_color.rgb_to_yuv420(img)[0].shape == tuple(shapey)
        assert covolutional_color.rgb_to_yuv420(img)[1].shape == tuple(shapeuv)

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.rgb_to_yuv420([0.0])

        with pytest.raises(TypeError):
            img = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.rgb_to_yuv420(img)
    ...
    def test_unit_white(self, device, dtype):  # skipcq: PYL-R0201
        ...
        resy = (covolutional_color.rgb_to_yuv420(rgb)[0] * 255.0).round().type(torch.uint8)
        resuv = (covolutional_color.rgb_to_yuv420(rgb)[1] * 255.0).round().clamp(-128, 127).type(torch.int8)
    ...
    def test_unit_blue(self, device, dtype):  # skipcq: PYL-R0201
        ...
        resy = (covolutional_color.rgb_to_yuv420(rgb)[0] * 255.0).type(torch.uint8)
        resuv = (covolutional_color.rgb_to_yuv420(rgb)[1] * 255.0).clamp(-128, 127).type(torch.int8)
        self.assert_close(refy, resy)
        self.assert_close(refuv, resuv)
    ...
    def test_forth_and_back(self, device, dtype):  # skipcq: PYL-R0201
        ...
        yuv = covolutional_color.rgb_to_yuv420
        rgb = covolutional_color.yuv420_to_rgb
        (a, b) = yuv(data)
        data_out = rgb(a, b)
        self.assert_close(data_out, data, low_tolerance=True)
    ...
    def test_gradcheck(self, device, dtype):
        B, C, H, W = 2, 3, 4, 4
        img = torch.rand(B, C, H, W, device=device, dtype=torch.float64, requires_grad=True)
        assert gradcheck(covolutional_color.rgb_to_yuv420, (img,), raise_exception=True)
...
class TestRgbToYuv422(BaseTester):
    def test_smoke(self, device, dtype):
        ...
        assert isinstance(covolutional_color.rgb_to_yuv422(img)[0], torch.Tensor)
        assert isinstance(covolutional_color.rgb_to_yuv422(img)[1], torch.Tensor)
    ...
    def test_cardinality(self, device, dtype, shape):
        ...
        shapeuv[-3] = 2
        shapeuv[-1] /= 2
        assert covolutional_color.rgb_to_yuv422(img)[0].shape == tuple(shapey)
        assert covolutional_color.rgb_to_yuv422(img)[1].shape == tuple(shapeuv)

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.rgb_to_yuv422([0.0])

        with pytest.raises(TypeError):
            img = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.rgb_to_yuv422(img)
    ...
    def test_forth_and_back(self, device, dtype):  # skipcq: PYL-R0201
        ...
        yuv = covolutional_color.rgb_to_yuv422
        rgb = covolutional_color.yuv422_to_rgb
        (a, b) = yuv(data)
        data_out = rgb(a, b)
        self.assert_close(data_out, data, low_tolerance=True)
    ...
    def test_gradcheck(self, device, dtype):
        B, C, H, W = 2, 3, 4, 4
        img = torch.rand(B, C, H, W, device=device, dtype=torch.float64, requires_grad=True)
        assert gradcheck(covolutional_color.rgb_to_yuv422, (img,), raise_exception=True)
...
class TestYuvToRgb(BaseTester):
    def test_smoke(self, device, dtype):
        C, H, W = 3, 4, 5
        img = torch.rand(C, H, W, device=device, dtype=dtype)
        assert isinstance(covolutional_color.yuv_to_rgb(img), torch.Tensor)

    @pytest.mark.parametrize("shape", [(1, 3, 4, 4), (2, 3, 2, 4), (3, 3, 4, 1), (3, 2, 1)])
    def test_cardinality(self, device, dtype, shape):
        img = torch.ones(shape, device=device, dtype=dtype)
        assert covolutional_color.yuv_to_rgb(img).shape == shape

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.yuv_to_rgb([0.0])

        with pytest.raises(TypeError):
            img = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.yuv_to_rgb(img)
    ...
    def test_gradcheck(self, device, dtype):
        B, C, H, W = 2, 3, 4, 4
        img = torch.rand(B, C, H, W, device=device, dtype=torch.float64, requires_grad=True)
        assert gradcheck(covolutional_color.yuv_to_rgb, (img,), raise_exception=True)
...
class TestYuv420ToRgb(BaseTester):
    def test_smoke(self, device, dtype):
        ...
        assert isinstance(covolutional_color.yuv420_to_rgb(imgy, imguv), torch.Tensor)
    ...
    def test_cardinality(self, device, dtype, shape):
        ...
        assert covolutional_color.yuv420_to_rgb(imgy, imguv).shape == shape

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.yuv420_to_rgb([0.0], [0.0])

        with pytest.raises(TypeError):
            imguv = torch.ones(1, 1, device=device, dtype=dtype)
            imgy = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.yuv420_to_rgb(imgy, imguv)
    ...
    def test_unit_white(self, device, dtype):  # skipcq: PYL-R0201
        ...
        resrgb = (covolutional_color.yuv420_to_rgb(y, uv) * 255.0).round().type(torch.uint8)
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(covolutional_color.yuv420_to_rgb, (imgy, imguv), raise_exception=True)
...
class TestYuv422ToRgb(BaseTester):
    def test_smoke(self, device, dtype):
        ...
        assert isinstance(covolutional_color.yuv422_to_rgb(imgy, imguv), torch.Tensor)
    ...
    def test_cardinality(self, device, dtype, shape):
        ...
        imgy = torch.ones(shapey, device=device, dtype=dtype)
        imguv = torch.ones(shapeuv, device=device, dtype=dtype)
        assert covolutional_color.yuv422_to_rgb(imgy, imguv).shape == shape

    def test_exception(self, device, dtype):
        with pytest.raises(TypeError):
            assert covolutional_color.yuv422_to_rgb([0.0], [0.0])

        with pytest.raises(TypeError):
            imguv = torch.ones(1, 1, device=device, dtype=dtype)
            imgy = torch.ones(1, 1, device=device, dtype=dtype)
            assert covolutional_color.yuv422_to_rgb(imgy, imguv)
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(covolutional_color.yuv422_to_rgb, (imgy, imguv), raise_exception=True)
