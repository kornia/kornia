...
class TestBinaryFocalLossWithLogits:
    ...
    def test_smoke_none(self, device, dtype):
        ...
        assert kornia.losses.binary_focal_loss_with_logits(logits, labels, alpha=0.5, gamma=2.0,
                                                           reduction="none").shape == (2, 3, 2)
...
class TestTotalVariation:
    ...
    @pytest.mark.parametrize(
        'input, expected', [
            (
                torch.tensor([[[[0.8756, 0.0920], [0.8034, 0.3107]], [[0.3069, 0.2981], [0.9399, 0.7944]],
                               [[0.6269, 0.1494], [0.2493, 0.8490]]],
                              [[[0.3256, 0.9923], [0.2856, 0.9104]], [[0.4107, 0.4387], [0.2742, 0.0095]],
                               [[0.7064, 0.3674], [0.6139, 0.2487]]]]), torch.tensor([5.0054283, 3.1870906])
            ),
            (
                torch.tensor([[[[0.1104, 0.2284, 0.4371], [0.4569, 0.1906, 0.8035]]],
    ...
    def test_tv_on_4d(self, device, dtype, input, expected):
        actual = kornia.losses.total_variation(input.to(device, dtype))
        assert_allclose(actual, expected.to(device, dtype), rtol=1e-4, atol=1e-4)
