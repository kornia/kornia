...
class SiftDescriptorWrapper:
    ...
    def detectAndCompute(self, img, mask):
        ...
        with torch.no_grad():
            img_t3 = kornia.utils.image_to_tensor(img_np, False).float() / 255.0
...
class TestDog:
    def test_dog(self):
        ...
        Hs_boat = [
            [
                [8.5828552e-01, 2.1564369e-01, 9.9101418e00],
                [-2.1158440e-01, 8.5876360e-01, 1.3047838e02],
                [2.0702435e-06, 1.2886110e-06, 1.0000000e00],
            ],
            [
                [5.6887079e-01, 4.6997572e-01, 2.5515642e01],
                [-4.6783159e-01, 5.6548769e-01, 3.4819925e02],
                [6.4697420e-06, -1.1704138e-06, 1.0000000e00],
            ],
            [
                [1.0016637e-01, 5.2319717e-01, 2.0587932e02],
                [-5.2345249e-01, 8.7390786e-02, 5.3454522e02],
                [9.4931475e-06, -9.8296917e-06, 1.0000000e00],
            ],
            [
                [4.2310823e-01, -6.0670438e-02, 2.6635003e02],
                [6.2730152e-02, 4.1652096e-01, 1.7460201e02],
                [1.5812849e-05, -1.4368783e-05, 1.0000000e00],
            ],
            [
                [2.9992872e-01, 2.2821975e-01, 2.2930182e02],
                [-2.3832758e-01, 2.4564042e-01, 3.6767399e02],
...
def rotate(img, sin_a, cos_a, rotation_index):
    ...
    H_gt = np.array([[cos_a, sin_a, 0.0], [-sin_a, cos_a, 0.0], [0.0, 0.0, 1.0]])

    box = np.array([[0.0, 0.0, 1.0], [0.0, h - 1, 1.0], [w - 1, 0.0, 1.0], [w - 1, h - 1, 1.0]])
    box2 = (H_gt @ box.T).T
    min_x = box2[:, 0].min()
    min_y = box2[:, 1].min()
