...
class TestBlackHat(utils.BaseTester):
    ...
    def test_value(self, device, dtype):
        ...
        expected = torch.tensor([[0.2, 0., 0.5], [0., 0.4, 0.], [0.3, 0., 0.6]],
                                device=device, dtype=dtype).unsqueeze(0).unsqueeze(0)
        assert_allclose(m.black_hat(input, kernel), expected)
    ...
    def test_exception(self, device, dtype):
        ...
        with pytest.raises(TypeError):
            assert m.black_hat([0.], kernel)

        with pytest.raises(TypeError):
            assert m.black_hat(input, [0.])

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4)
            assert m.black_hat(test, kernel)

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4)
            assert m.black_hat(input, test)
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(m.black_hat, (input, kernel), raise_exception=True)
