...
class TestBlackHat(utils.BaseTester):
    ...
    def test_cardinality(self, device, dtype, shape, kernel):
        ...
        assert morph.black_hat(img, krnl).shape == shape
    ...
    def test_value(self, device, dtype):
        ...
        expected = torch.tensor([[0.2, 0., 0.5], [0., 0.4, 0.], [0.3, 0., 0.6]],
                                device=device, dtype=dtype)[None, None, :, :]
        assert_allclose(morph.black_hat(input, kernel), expected)
    ...
    def test_exception(self, device, dtype):
        ...
        with pytest.raises(TypeError):
            assert morph.black_hat([0.], kernel)

        with pytest.raises(TypeError):
            assert morph.black_hat(input, [0.])

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4, device=device, dtype=dtype)
            assert morph.black_hat(test, kernel)

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4, device=device, dtype=dtype)
            assert morph.black_hat(input, test)
    ...
    def test_module(self, device, dtype):
        ...
        fcn1 = morph.black_hat
        assert_allclose((ops1(ops2(img)) - img), fcn1(img, krnl))
