...
class TestTopHat(utils.BaseTester):
    ...
    def test_cardinality(self, device, dtype, shape, kernel):
        ...
        assert morph.top_hat(img, krnl).shape == shape
    ...
    def test_value(self, device, dtype):
        ...
        expected = torch.tensor([[0., 0.5, 0.], [0.2, 0., 0.5], [0., 0.5, 0.]],
                                device=device, dtype=dtype)[None, None, :, :]
        assert_allclose(morph.top_hat(input, kernel), expected)
    ...
    def test_exception(self, device, dtype):
        ...
        with pytest.raises(TypeError):
            assert morph.top_hat([0.], kernel)

        with pytest.raises(TypeError):
            assert morph.top_hat(input, [0.])

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4, device=device, dtype=dtype)
            assert morph.top_hat(test, kernel)

        with pytest.raises(ValueError):
            test = torch.ones(2, 3, 4, device=device, dtype=dtype)
            assert morph.top_hat(input, test)
    ...
    def test_gradcheck(self, device, dtype):
        ...
        assert gradcheck(morph.top_hat, (input, kernel), raise_exception=True)
    ...
    def test_module(self, device, dtype):
        ...
        fcn1 = morph.top_hat
        assert_allclose((img - ops2(ops1(img))), fcn1(img, krnl))
