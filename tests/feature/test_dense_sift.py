...
from kornia.feature import DenseSIFTFeature, get_laf_orientation, laf_from_center_scale_ori

from testing.base import BaseTester


class TestDenseSIFTFeature(BaseTester):
    def test_shape_norm_and_empty(self, device, dtype):
        ...
        feature = DenseSIFTFeature().to(device, dtype)
        oriented, descriptors = feature(image, lafs)
        ...
        empty_lafs, empty_descriptors = feature(image, lafs[:, :0])
    ...
    def test_horizontal_gradient_keeps_orientation(self, device, dtype):
        ...
        oriented, _ = DenseSIFTFeature(rootsift=False).to(device, dtype)(image, lafs)
        self.assert_close(
    ...
    def test_flat_input_has_finite_backward(self, device, dtype):
        ...
        xy = torch.tensor([[[32.0, 32.0]]], device=device, dtype=dtype)
        lafs = laf_from_center_scale_ori(xy, torch.full((1, 1, 1, 1), 8.0, device=device, dtype=dtype))
        _, descriptors = DenseSIFTFeature().to(device, dtype)(image, lafs)
    ...
    def test_odd_spatial_pooling(self, device, dtype):
        ...
        xy = torch.tensor([[[32.0, 32.0]]], device=device, dtype=dtype)
        lafs = laf_from_center_scale_ori(xy, torch.full((1, 1, 1, 1), 8.0, device=device, dtype=dtype))
        _, descriptors = DenseSIFTFeature(spatial_bin_size=3).to(device, dtype)(image, lafs)
    ...
    def test_invalid_laf_and_mixed_laf_dtype(self, device, dtype):
        ...
        oriented, descriptors = DenseSIFTFeature().to(device, dtype)(image, lafs)
    ...
    def test_upright_preserves_laf_orientation(self, device, dtype):
        ...
        oriented, _ = DenseSIFTFeature(upright=True).to(device, dtype)(image, lafs)
    ...
    def test_affine_gradient_is_canonicalized(self, device, dtype):
        ...
        oriented, descriptors = DenseSIFTFeature(rootsift=False).to(device, dtype)(image, lafs)
    ...
    def test_nonfinite_frame_backward(self, device, dtype):
        ...
        _, desc = DenseSIFTFeature().to(device, dtype)(image, lafs)
        desc.sum().backward()
    ...
    def test_gradcheck(self, device):
        ...
        feature = DenseSIFTFeature(upright=True, spatial_bin_size=3).to(device, torch.float64)
        self.gradcheck(lambda image: feature(image, lafs)[1], (image,))
    ...
    def test_empty_batch(self, device, dtype):
        ...
        oriented, desc = DenseSIFTFeature().to(device, dtype)(image, lafs)
