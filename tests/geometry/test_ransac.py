...
# and _H_TRUE is the planar homography of the batch-8 two-view fixture (float64 DLT fit of its xp1 -> xp2, 7
# significant digits); every entry is distinct and non-zero, so swapping the images changes the model.
_MATCHES_1 = [
...
# Twelve pixel matches of the batch-8 two-view fixture (a non-planar scene, K1 != K2, R and t off every axis),
# rounded to 0.01 px: np.round(np.load("fixture.npz")["x1"], 2) and ["x2"].
_SCENE_1 = [
    [318.64, 292.32], [219.52, 150.04], [317.09, 310.64], [358.6, 158.12], [168.74, 100.38], [326.6, 266.96],
    [215.08, 135.98], [483.37, 161.54], [289.85, 218.8], [154.51, 93.31], [264.62, 316.61], [254.08, 104.95],
]  # fmt: skip
_SCENE_2 = [
    [235.4, 269.27], [130.41, 117.07], [217.99, 283.87], [261.49, 135.6], [93.43, 63.07], [242.34, 244.53],
    [135.21, 103.96], [389.61, 151.37], [215.8, 196.24], [89.28, 56.36], [168.56, 287.91], [167.81, 73.56],
]  # fmt: skip
_HALF_LINES = (
...
def _cpu_only(device: torch.device) -> None:
    if device.type != "cpu":
        pytest.skip("RANSAC's batched minimal solvers abort the MPS process (#4204); the convention pins run on CPU")
...
class TestConventionRANSAC(BaseTester):
    def test_convention_ransac_model_maps_kp1_to_kp2(self, device, dtype):
        _cpu_only(device)
        ...
        # A (3, 3) model in the input dtype and an (N,) bool mask; the model maps kp1 onto kp2, and the control
        # (applying it to kp2) misses kp1 by about 213 px.
        assert model.shape == (3, 3) and model.dtype == dtype
        ...
        assert model.dtype == dtype and bool((model == 0).all())
    ...
    def test_convention_ransac_inl_th_is_pixels_for_point_models(self, model_type, device, dtype):
        ...
        if model_type == "homography":
            ...
            # absorb part of the displacement into a compromise model, so the flip lies somewhat below 10 px; both
        ...
        # the least-squares F of the twelve matches (exact up to the 0.01 px rounding of the literals), so the
        # threshold is observed without sampling.
        kp1 = torch.tensor(_SCENE_1, device=device, dtype=dtype)
        kp2 = torch.tensor(_SCENE_2, device=device, dtype=dtype)
