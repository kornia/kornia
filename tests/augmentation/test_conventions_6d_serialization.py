...
class TestConventionMix3DSerialization(BaseTester):
    ...
    def test_convention_numeric_config_pickle_and_deepcopy_preserve_replay(self, name, kwargs):
        ...
        augmentation = getattr(K, name)(p=1.0, **kwargs)
        ...
        for restored in (pickle.loads(pickle.dumps(augmentation)), copy.deepcopy(augmentation)):  # noqa: S301
            ...
            reference_params = augmentation.forward_parameters(image.shape)
            torch.manual_seed(31)
            restored_params = restored.forward_parameters(image.shape)
