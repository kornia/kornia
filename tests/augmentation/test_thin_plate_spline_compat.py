...
def test_corrected_grid_opt_in(device: torch.device, dtype: torch.dtype) -> None:
    ...
    with pytest.warns(DeprecationWarning, match="use_correct_grid=True") as record:
        legacy_output = legacy.apply_transform(image, params, legacy.flags)
    warning = str(record[0].message)
    assert "warp_image_tps" in warning
    assert "RandomThinPlateSpline" in warning
...
def test_compile_legacy_and_corrected_grids(optimizer_backend: str) -> None:
    ...
    compiled_legacy = torch.compile(legacy, backend=optimizer_backend, fullgraph=True)(image, src, kernel, affine)
    compiled_corrected = torch.compile(corrected, backend=optimizer_backend, fullgraph=True)(image, src, kernel, affine)
