...
class TestIntensityColourConventions(BaseTester):
    ...
    def test_convention_random_planckian_jitter_scales_red_and_blue_then_clamps_above(self, device, dtype):
        ...
        # Explicit .to(dtype) here is a no-op since the fix for #4574 (below): the output already
        # matches the input's own dtype.
    ...
    # Fixed for #4574: apply_transform now casts the `pl` table to the input's own dtype before
    # indexing it, rather than only to its device, so the output always matches the input's dtype --
    # regardless of what dtype (if any) the module itself has separately been cast to.
    # Snippet used to generate expected:
    ...
    #   torch.manual_seed(0)
    #   print(K.RandomPlanckianJitter(p=1.0).to(dtype=torch.bfloat16)(torch.rand(2, 3, 4, 4).half()).dtype)
    # executed 2026-09-17 (torch 2.14.0, cpu) -> `float16`, `bfloat16`, `float64`; `float16`.
    def test_convention_random_planckian_jitter_output_dtype_matches_input_4574(self, device, dtype):
        torch.manual_seed(_FIXTURE_SEED)
        image = torch.rand(2, 3, 4, 4).to(device=device, dtype=dtype)
        torch.manual_seed(_FORWARD_SEED)
        out = K.RandomPlanckianJitter(p=1.0)(image)
        assert out.dtype == dtype
        # Casting the module to some OTHER dtype first makes no difference: the table is re-cast to
        # the input's dtype on every forward call.
        if dtype in _HALF:
