...
class TestConventionPatchSequential(BaseTester):
    """Convention checks and pins for documented `PatchSequential` limitations."""
    ...
    def test_wart_patch_sequential_augments_b_times_c_patch_rows_4421(self, device, dtype):
        ...
        # bounds for dimension 0 with size 2"). The deliberately invalid-index subcase stays on CPU: CUDA
        # reports it through a device assertion that poisons the shared CUDA context. B=2, C=3 on a
        # non-square 8x12 image with a (2, 3)
        ...
        with pytest.raises(IndexError, match="is out of bounds for dimension 0 with size 2"):
            K.PatchSequential(K.RandomHorizontalFlip(p=1.0), grid_size=(1, 1), patchwise_apply=False)(
                torch.rand(2, 3, 8, 12, device="cpu", dtype=torch.float32)
            )
