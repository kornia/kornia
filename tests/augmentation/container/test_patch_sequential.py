...
class TestConventionPatchSequential:
    ...
    def test_wart_patch_sequential_augments_b_times_c_patch_rows_4421(self, device, dtype):
        ...
        # accepts either type and does not pin the index; B=2, C=3, grid (2, 2) -> the 8 patches come back as
        # ['flip', 'flip', 'flip', 'flip', 'flip', 'flip', 'same', 'same'] - the last two patches of the
        # second sample are untouched. The fix lands in the repair window; do not "correct" this pin here.
        ...
        with pytest.raises((IndexError, RuntimeError), match="is out of bounds for dimension 0 with size 2"):
            K.PatchSequential(K.RandomHorizontalFlip(p=1.0), grid_size=(1, 1), patchwise_apply=False)(
                torch.rand(2, 3, 8, 8, device=device, dtype=dtype)
            )
        seq = K.PatchSequential(K.RandomHorizontalFlip(p=1.0), grid_size=(2, 2), patchwise_apply=False)
        ...
        for b in range(2):
            for rows in ((0, 4), (4, 8)):
                for cols in ((0, 4), (4, 8)):
                    patch_in = x[b, :, rows[0] : rows[1], cols[0] : cols[1]]
                    patch_out = out[b, :, rows[0] : rows[1], cols[0] : cols[1]]
                    if torch.equal(patch_out, patch_in.flip(-1)):
                        ...
                        states.append("other")
        assert states == ["flip"] * 6 + ["same", "same"]
