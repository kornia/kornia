...
- `.claude/skills/kornia-review-loop/SKILL.md` — responding to a review finding whose fix touches `kornia/`, a docstring/docs sentence, or a test's expected value/skip/xfail (a fix that only reorganises tests takes the short path it names): triage against the merge-base with the ref the PR actually targets (`gh pr view --json baseRefName`, not an assumed `origin/main`), fix defects only, delta self-review, `pixi run verify-delta`, then push once.
