...
- Some modules document a known defect inline, in a `.. warning::` block ending `Tracked in #NNNN` — `kornia/geometry/conversions.py` most heavily, plus `crop2d.py`, `pyramid.py` and `thin_plate_spline.py`. That prose is part of the fix's blast radius: **before merging a change that closes one of those issues, grep the module for the issue number; if it is still there, the change is incomplete.** A clean `git merge` will not tell you — #3999 fixed the three homography shape guards without touching the docstrings, and `main` then shipped rendered API documentation promising the behavior that had just been removed. The paired test pins (`test_wart_*_<issue>` and `test_convention_*_<issue>` in `tests/`) have to be flipped in the same change for the same reason.
