...
- `tests/api_surface.json` is a checked-in inventory of the public names of the stable-core modules, and `tests/test_api_surface.py` fails when one of them disappears — the deliberate review moment the stability policy (`docs/source/get-started/stability.rst`) requires. Removing or renaming a public name means editing that JSON in the same change; additions do not fail the test, and `tests.test_api_surface.regenerate()` rewrites the inventory from the live library once new public API has landed. The `Import Surface` workflow reads the same edit: a name dropped from an `__all__` under `kornia/` fails `check_import_surface.py` unless the same change removes that name from the inventory entry for that module or one of its ancestor packages. The inventory edit has to be in the PR that makes the removal: dropping a module's whole key fails that check outright (the key is what puts the module in `test_no_public_name_removed`'s parametrization — record losing every public name with an empty list instead), and so does recording a removal the same change does not make. A package-level entry only speaks for the module the package actually re-exports that name from. Names no tracked package records have no entry to drop and so no recorded-removal path at all (#4236).
